...
import { sendMonitoringAlert, sendSuccessReport, isMailerConfigured } from '../utils/mailer.js';
...
const API_KEYS = {
  ...
  bgg: process.env.TEST_BGG_TOKEN || '' // BoardGameGeek Bearer token
};
...
const PROVIDER_TESTS = [
  ...
  // BoardGameGeek (nécessite Bearer token)
  {
    provider: 'BGG',
    route: '/bgg/search',
    params: { q: 'catan' },
...
export async function runAllTests() {
  ...
  if (failures.length > 0) {
    ...
    // Tout va bien - envoyer un rapport si activé
    const sendSuccessEnabled = process.env.SEND_SUCCESS_REPORT === 'true';
    if (sendSuccessEnabled && isMailerConfigured()) {
      log.info('✅ Tous les tests passés - envoi du rapport de succès');
      const sent = await sendSuccessReport(report);
      if (sent) {
        log.info('📧 Rapport de succès envoyé');
      } else {
        log.error('❌ Échec envoi rapport de succès');
      }
    } else if (sendSuccessEnabled) {
      log.warn('📧 Mailer non configuré - pas de rapport de succès');
    }
  }
