 ...
 */
export async function translateGenres(genres, lang) {
  ...
  const translatedGenres = await Promise.all(
    genres.map(g => translateGenre(g, lang))
  );
...
export async function translateToEnglish(text, sourceLang = null) {
  ...
  // Si déjà en anglais, ne pas traduire
  if (sourceLang === 'en' || isLikelyEnglish(text)) {
    return result;
  }
  
  try {
    log.debug(`Traduction vers anglais: "${text}"`);
    ...
    const response = await fetch(`${AUTO_TRAD_URL}/translate`, {
      ...
      signal: AbortSignal.timeout(3000) // Timeout 3s (plus court car critique pour la recherche)
    });
    ...
    if (data.results && data.results[0] && data.results[0].translated) {
      const translatedText = data.results[0].translated;
      const detectedLang = data.results[0].detected_lang || 'unknown';
