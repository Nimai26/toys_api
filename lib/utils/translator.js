...
export async function translateText(text, destLang, options = {}) {
  ...
  try {
    ...
    // Timeout dynamique : 15s de base + 1s par 500 caractères (max 60s)
    const timeoutMs = Math.min(15000 + Math.floor(text.length / 500) * 1000, 60000);
    ...
    if (err.name === 'TimeoutError') {
      log.warn(`Timeout traduction (${Math.floor(timeoutMs/1000)}s)`);
    } else {
