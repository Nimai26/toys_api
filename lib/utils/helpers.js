/**
 * lib/utils/helpers.js - Fonctions utilitaires communes
 * toys_api v1.26.0
 * 
...
export function decodeHtmlEntities(text) {
  ...
  return text
    ...
    .replace(/&nbsp;/g, ' ')
    .replace(/&#160;/g, ' ')
    // Entités HTML nommées accentuées (français, etc.)
    .replace(/&eacute;/g, 'é')
    .replace(/&egrave;/g, 'è')
    .replace(/&ecirc;/g, 'ê')
    .replace(/&euml;/g, 'ë')
...
/**
 * Nettoyer le HTML en supprimant toutes les balises et en décodant les entités
...
export function stripHtml(html) {
  ...
  return decodeHtmlEntities(
    html
      ...
      .replace(/<br\s*\/?>/gi, '\n')
      // Remplacer </p>, </div>, </li> par des sauts de ligne
...
/**
 * Construit une URL de proxy absolue pour contourner les protections anti-hotlinking
 * Utilise TOY_API_BASE_URL si défini, sinon retourne une URL relative
 * 
 * @param {string} proxyPath - Le chemin du proxy (ex: "/bgg_scrape/proxy")
 * @param {string} targetUrl - L'URL cible à proxifier
 * @returns {string} URL de proxy complète (absolue si API_BASE_URL défini, relative sinon)
 * 
 * @example
 * // Avec TOY_API_BASE_URL="http://toys_api:3000"
 * buildProxyUrl('/bgg_scrape/proxy', 'https://example.com/image.jpg')
 * // => "http://toys_api:3000/bgg_scrape/proxy?url=https%3A%2F%2Fexample.com%2Fimage.jpg"
 * 
 * // Sans TOY_API_BASE_URL (URL relative)
 * buildProxyUrl('/bgg_scrape/proxy', 'https://example.com/image.jpg')
 * // => "/bgg_scrape/proxy?url=https%3A%2F%2Fexample.com%2Fimage.jpg"
 */
export function buildProxyUrl(proxyPath, targetUrl) {
  if (!targetUrl) return null;
  const relativeUrl = `${proxyPath}?url=${encodeURIComponent(targetUrl)}`;
