...
/**
 * Envoie un rapport de succès (tout va bien)
 * @param {object} report - Rapport de monitoring
 * @param {number} report.total - Total de tests
 * @param {number} report.passed - Tests réussis
 * @param {Date} report.timestamp - Date du test
 * @param {Array} report.results - Résultats détaillés
 */
export async function sendSuccessReport(report) {
  const { total, passed, timestamp, results } = report;
  
  const subject = `✅ Toys API - Rapport monitoring: ${passed}/${total} tests OK`;
  
  // Grouper les résultats par provider
  ...
  const providerSummary = Object.entries(byProvider)
    .map(([provider, tests]) => {
      const avgTime = Math.round(tests.reduce((a, t) => a + t.duration, 0) / tests.length);
      return `✅ ${provider}: ${tests.length} test(s), ~${avgTime}ms`;
    })
    .join('\n');
  
  const text = `
✅ RAPPORT MONITORING - Toys API
================================
...
---
Cet email est envoyé automatiquement toutes les ${process.env.HEALTHCHECK_INTERVAL_HOURS || 10}h.
Pour désactiver: SEND_SUCCESS_REPORT=false
`;
...
<head>
  <style>
    ...
    .stats { background: #f8f9fa; padding: 15px; margin: 15px 0; border-radius: 5px; }
    .provider { background: #d4edda; border-left: 4px solid #28a745; padding: 8px 12px; margin: 5px 0; }
    .provider-name { font-weight: bold; color: #155724; }
    .provider-time { color: #666; font-size: 0.9em; }
    .footer { color: #666; font-size: 0.8em; margin-top: 20px; }
  </style>
</head>
<body>
  <div class="header">
    <h2>✅ Toys API - Tout fonctionne !</h2>
  </div>
  
  <div class="stats">
    <p><strong>Date:</strong> ${timestamp.toLocaleString('fr-FR')}</p>
    <p><strong>Tests:</strong> ${passed}/${total} réussis</p>
    <p><strong>Statut:</strong> 🟢 Tous les providers opérationnels</p>
  </div>
  
  <h3>Résumé par provider:</h3>
  ${Object.entries(byProvider).map(([provider, tests]) => {
    const avgTime = Math.round(tests.reduce((a, t) => a + t.duration, 0) / tests.length);
    return `
      <div class="provider">
        <span class="provider-name">✅ ${provider}</span>
        <span class="provider-time"> - ${tests.length} test(s), ~${avgTime}ms</span>
      </div>
    `;
  }).join('')}
  
  <div class="footer">
    <p>Rapport automatique toutes les ${process.env.HEALTHCHECK_INTERVAL_HOURS || 10}h.</p>
    <p>Pour désactiver: <code>SEND_SUCCESS_REPORT=false</code></p>
  </div>
</body>
</html>
`;
