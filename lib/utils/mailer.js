...
const SMTP_CONFIG = {
  ...
  secure: process.env.SMTP_SECURITY === 'ssl', // true pour 465, false pour autres ports
  auth: {
...
export async function sendMonitoringAlert(report) {
  ...
  const subject = `⚠️ Toys API - ${failures.length} provider(s) en échec`;
