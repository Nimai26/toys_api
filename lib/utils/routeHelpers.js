...
export function extractStandardParams(req) {
  const lang = normalizeLanguage(req.query.lang || req.query.language || req.query.locale || 'fr');
  
  // Paramètre pour forcer le refresh (ignorer le cache)
  // Accepte: refresh=true, cache=false, db=false, nocache=true
  const forceRefresh = parseBoolean(req.query.refresh) || 
                       parseBoolean(req.query.nocache) ||
  ...
  return {
    ...
    // Pays/région (dérivé de lang pour uniformité)
    // Utilisé par Amazon pour le marketplace
    country: lang,
    
    // Force le refresh depuis l'API (ignore le cache PostgreSQL)
    // Usage: &refresh=true ou &cache=false ou &db=false
...
export function parseDetailUrl(detailUrl) {
  ...
  try {
    ...
    let decoded = decodeURIComponent(detailUrl);
    
    // Supprimer le protocole toys:// si présent
    decoded = decoded.replace(/^toys:\/\//, '');
 ...
 * @param {Object} [options.cacheMatch] - Infos de correspondance cache (similarité)
 * @returns {Object}
 */
export function formatSearchResponse({ items, provider, query, pagination = {}, meta = {}, cacheMatch = null }) {
  const response = {
    success: true,
    provider,
    query,
  ...
  // Ajouter les infos de cache match si disponibles (recherche par similarité)
  if (cacheMatch && cacheMatch.type === 'similarity') {
    response.cacheMatch = {
      type: 'similarity',
      score: Math.round(cacheMatch.score * 100),
      originalQuery: cacheMatch.originalQuery,
      message: `Résultats trouvés pour "${cacheMatch.originalQuery}" (${Math.round(cacheMatch.score * 100)}% similaire)`
    };
  }
