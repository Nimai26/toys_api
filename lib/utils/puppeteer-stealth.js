...
export async function stealthGet(url, options = {}) {
  const {
    ...
    fullPageScroll = false, // Scroller jusqu'en bas de la page pour charger le lazy loading
    useProxy = USE_VPN_PROXY, // Utiliser le proxy VPN (défaut: true)
    fastMode = false, // Désactiver les délais de simulation humaine (sites qui ne bloquent pas)
  } = options;
  ...
  try {
    ...
    // 1. Attendre un délai aléatoire (réduit en mode rapide)
    await new Promise(resolve => setTimeout(resolve, fastMode ? 100 : (1500 + Math.random() * 2000)));
    
    // 2. Scroll (complet si fullPageScroll=true, sinon aléatoire)
    if (fullPageScroll) {
      // Scroller progressivement jusqu'en bas pour charger le lazy loading
      try {
        await page.evaluate(async () => {
          await new Promise((resolve) => {
            let totalHeight = 0;
            const distance = 100; // Distance de scroll par itération
            ...
            }, 100); // Scroll toutes les 100ms
          });
        });
        log.info('Scroll complet effectué (lazy loading)');
      } catch (err) {
        log.warn(`Erreur lors du scroll complet: ${err.message}`);
      }
      // Attendre que le lazy loading charge les éléments
      await new Promise(resolve => setTimeout(resolve, 2000));
    } else {
      // Scroll aléatoire pour simuler un utilisateur
      try {
        await page.evaluate(() => {
      ...
      await new Promise(resolve => setTimeout(resolve, fastMode ? 100 : (500 + Math.random() * 1000)));
    }
    ...
    await new Promise(resolve => setTimeout(resolve, fastMode ? 50 : (300 + Math.random() * 500)));

    // 4. Attendre le lazy-loading des images - skip en mode rapide
    if (!fullPageScroll && !fastMode) {
      await new Promise(resolve => setTimeout(resolve, 2000));
    }
