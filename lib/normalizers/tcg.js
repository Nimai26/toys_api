...
export async function normalizePokemonCardOfficial(rawCard, options = {}) {
  ...
  const cardId = `${rawCard.set}-${rawCard.number}`;

  // === Description (format riche similaire à Digimon) ===
  let description = '';
  
  // Type de carte + HP
  if (rawCard.cardType) {
    description += `**${rawCard.cardType}**`;
  }
  if (rawCard.energyType) {
    description += ` - ${rawCard.energyType}`;
  }
  if (rawCard.hp) {
    description += `\n\nHP: ${rawCard.hp}`;
  }
  ...
  if (rawCard.attacks && rawCard.attacks.length > 0) {
    description += '\n';
    rawCard.attacks.forEach(attack => {
      const energyIcons = attack.energyCost ? attack.energyCost.map(e => `[${e}]`).join(' ') : '';
      const damage = attack.damage ? ` ${attack.damage}` : '';
      description += `\n\n**${attack.name}** ${energyIcons}${damage}`;
  ...
  const stats = [];
  if (rawCard.weakness) {
    stats.push(`Faiblesse: ${rawCard.weakness.type} ${rawCard.weakness.value}`);
  }
  if (rawCard.resistance) {
    stats.push(`Résistance: ${rawCard.resistance.type} ${rawCard.resistance.value}`);
  }
  if (rawCard.retreatCost !== undefined && rawCard.retreatCost !== null) {
    stats.push(`Coût de Retraite: ${rawCard.retreatCost}`);
  }
  ...
  // Format Digimon-like avec structure complète
  const baseUrl = API_BASE_URL || `http://${process.env.IP_HOST || '10.110.1.1'}:${process.env.TOYS_API_PORT || '3000'}`;
  
  return {
    id: cardId,
    source: 'tcg_pokemon',
    collection: 'Pokémon Trading Card Game',
    name: rawCard.name || null,
    description: description.trim() || null,
    images: [
      {
        type: 'default',
        url: rawCard.imageUrl ? `${baseUrl}/proxy/image?url=${encodeURIComponent(rawCard.imageUrl)}` : null,
        originalUrl: rawCard.imageUrl || null
      }
    ],
    attributes: {
      type: rawCard.cardType || null,
      energy_type: rawCard.energyType || null,
      hp: rawCard.hp || null,
      attacks: rawCard.attacks || [],
      weakness: rawCard.weakness || null,
      resistance: rawCard.resistance || null,
      retreat_cost: rawCard.retreatCost || 0,
      artist: rawCard.illustrator || null
    },
    set: {
      name: rawCard.setName || null,
      number: rawCard.setNumber || null,
      symbol: rawCard.setSymbol || null,
      series: 'Pokémon Trading Card Game'
    },
    pretty_url: rawCard.name ? `${rawCard.name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-${cardId}` : cardId,
    date_added: new Date().toISOString().split('T')[0] + ' ' + new Date().toTimeString().split(' ')[0]
  };
}
