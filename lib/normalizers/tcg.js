...
export async function normalizePokemonSearchOfficial(rawData, options = {}) {
  ...
  // Les résultats de recherche contiennent juste set/number/url
...
export async function normalizePokemonCardOfficial(rawCard, options = {}) {
  ...
  if (rawCard.imageUrl) {
    images.push({
      url: rawCard.imageUrl,
      thumbnail: rawCard.imageUrl,
      caption: 'normal',
      is_main: true
    });
  }
  ...
  if (rawCard.attacks && rawCard.attacks.length > 0) {
    const attacksText = rawCard.attacks.map(a => {
      ...
      return `**${a.name}** ${damage}: ${a.text || ''}`.trim();
    }).join('\n\n');
    description += attacksText;
  }
  ...
  const attributes = {
    hp: rawCard.hp || null,
    types: rawCard.energyCost ? [rawCard.energyCost[0]] : [],
    attacks: rawCard.attacks || [],
    weaknesses: rawCard.weakness ? [{
      type: rawCard.weakness.type,
      value: rawCard.weakness.value
    }] : [],
    resistances: rawCard.resistance ? [{
      type: rawCard.resistance.type,
      value: rawCard.resistance.value
    }] : [],
  ...
  return {
    ...
    card_type: rawCard.cardType || 'Pokémon',
    supertype: 'Pokémon',
    subtypes: [rawCard.cardType || 'Pokémon de base'],
    images,
    description,
    flavor_text: null,
    set: {
      id: rawCard.set,
      name: rawCard.setName || rawCard.set,
      code: rawCard.set,
      series: rawCard.setName || null,
      total: rawCard.setTotal || null,
      release_date: null
    },
    card_number: rawCard.cardNumber || `${rawCard.number}/${rawCard.setTotal || '?'}`,
