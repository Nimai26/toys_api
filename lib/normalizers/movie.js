...
/**
 * Extrait et normalise les genres depuis différents formats de données
 * Gère le format objet de translateGenres: { genres: [...], genresOriginal: [...], genresTranslated: boolean }
 * Et le format aplati: genres (array), genresOriginal (array), genresTranslated (array|boolean)
 * @param {object} data - Données du provider
 * @returns {{ genres: string[], genres_original: string[], genres_translated: string[]|null }}
 */
export function extractGenres(data) {
  ...
  // Format 1: data.genres est un objet de translateGenres { genres: [...], genresOriginal: [...], genresTranslated: boolean }
  if (typeof data.genres === 'object' && !Array.isArray(data.genres) && data.genres?.genres) {
    genres = data.genres.genres || [];
    genresOriginal = data.genres.genresOriginal || data.genresOriginal || genres;
    genresTranslated = data.genres.genresTranslated ? genres : null;
  }
  // Format 2: genresTranslated est un boolean true (IMDB style) - genres contient les traduits
  else if (data.genresTranslated === true) {
    genres = Array.isArray(data.genres) ? data.genres : [];
    genresOriginal = Array.isArray(data.genresOriginal) ? data.genresOriginal : genres;
    genresTranslated = genres;
  }
  // Format 3: genresTranslated est un tableau (déjà aplati)
  else if (Array.isArray(data.genresTranslated)) {
    genres = data.genresTranslated;
    genresOriginal = Array.isArray(data.genresOriginal) ? data.genresOriginal : (Array.isArray(data.genres) ? data.genres : []);
    genresTranslated = data.genresTranslated;
  }
  // Format 4: tableau simple sans traduction
  else {
    genres = Array.isArray(data.genres) ? data.genres : [];
    genresOriginal = Array.isArray(data.genresOriginal) ? data.genresOriginal : genres;
    genresTranslated = null;
  }
  
  return { genres, genres_original: genresOriginal, genres_translated: genresTranslated };
}
...
export function normalizeImdbMovieDetail(data) {
  ...
  // plotTranslated est un boolean, pas le texte traduit
  ...
  if (data.images && Array.isArray(data.images)) {
    data.images.forEach(img => {
      images.push({ type: 'backdrop', url: img });
    });
  }
