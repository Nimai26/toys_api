...
export function normalizeAmazonVideogameSearchItem(item) {
  ...
  const sourceUrl = item.url || `https://${marketplace.domain}/dp/${item.asin}`;
  
  return {
    // Champs de base pour la recherche (compatibilité frontend)
    type: 'videogame',
    source: 'amazon_videogames',
    sourceId: item.asin,
    name: item.title || '',
    name_original: item.title || '',
    image: imageUrl,
    src_url: sourceUrl,
    detailUrl: `/amazon_videogames/details?detailUrl=/amazon_videogames/product/${item.asin}`,
    description: null,
    year: null,
    
    // Champs enrichis format videogame
    provider: 'amazon',
...
export function normalizeAmazonBookSearchItem(item) {
  ...
  return {
    ...
    image: imageUrl,
    src_url: sourceUrl,
    detailUrl: `/amazon_books/details?detailUrl=/amazon_books/product/${item.asin}`,
...
export function normalizeAmazonMovieSearchItem(item) {
  ...
  return {
    // Champs de compatibilité frontend
    source: 'amazon_movies',
    sourceId: item.asin,
    name: item.title || '',
    image: imageUrl,
    src_url: sourceUrl,
    detailUrl: `/amazon_movies/details?detailUrl=/amazon_movies/product/${item.asin}`,
...
export function normalizeAmazonMovieDetail(data) {
  ...
  const images = [];
  if (data.image) {
    images.push({ type: 'poster', url: data.image });
  }
  if (data.images && Array.isArray(data.images)) {
    data.images.forEach((url, index) => {
      images.push({ type: index === 0 && !data.image ? 'poster' : 'screenshot', url });
    });
  }
  
  // Description avec support traduction
  const description = data.description || null;
  const descriptionOriginal = data.descriptionOriginal || null;
  const descriptionTranslated = data.descriptionTranslated || null;
  
  // Genres avec support traduction (format unifié)
  const genres = data.genresTranslated || data.genres || data.genresOriginal || [];
  const genresOriginal = data.genresOriginal || data.genres || [];
  const genresTranslated = data.genresTranslated || null;
  ...
  return {
    ...
    genres_translated: genresTranslated,
    keywords: [],
...
export function normalizeAmazonMusicSearchItem(item) {
  ...
  return {
    ...
    image: imageUrl,
    src_url: sourceUrl,
    detailUrl: `/amazon_music/details?detailUrl=/amazon_music/product/${item.asin}`,
...
export function normalizeAmazonToySearchItem(item) {
  ...
  return {
    ...
    image: imageUrl,
    src_url: sourceUrl,
    detailUrl: `/amazon_toys/details?detailUrl=/amazon_toys/product/${item.asin}`,
