...
/**
 * Normalise les images BGG en tableau [{url, type}]
 * Utilise les URLs proxy par défaut pour contourner l'anti-hotlinking BGG
 * @param {object} raw - Données brutes avec image et imageProxy
 * @returns {Array<{url: string, type?: string}>} - Tableau d'images normalisées
 */
export function normalizeBggImages(raw) {
  if (!raw) return [];
  ...
  // URL proxy par défaut (contourne protection BGG)
  const primaryUrl = raw.imageProxy || raw.image;
  addImage(primaryUrl, 'primary');
  
  // URL directe conservée comme fallback
  if (raw.image && raw.image !== primaryUrl) {
    addImage(raw.image, 'primary_direct');
  }
...
export function normalizeBGGDetail(raw, options = {}) {
  ...
  return {
    ...
    // Images - normalisées en tableau [{url, type}], URLs proxy par défaut pour contourner anti-hotlinking BGG
    images: normalizeBggImages(raw),
