...
export function normalizeColekaDetail(data) {
  ...
  return {
    ...
    brand: data.brand || (data.brands && data.brands[0]) || null,
    brands: data.brands || [],
...
export function normalizeTransformerlandSearchItem(item) {
  // Utiliser l'année fournie par le provider, ou l'extraire du subgroup
  let year = item.year || null;
  if (!year && item.subgroup) {
    const yearMatch = item.subgroup.match(/\((\d{4})\)/);
...
export function normalizeTransformerlandDetail(data) {
  ...
  return {
    ...
    // Normaliser les images au format [{ url }] en éliminant les doublons
    images: normalizeImages(data.images),
