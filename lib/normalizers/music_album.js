...
/**
 * Normalise les images d'album en tableau [{url, type}]
 * @param {object} images - Images object avec thumbnail, cover, cover_large etc.
 * @returns {Array<{url: string, type?: string}>} - Tableau d'images normalisées
 */
export function normalizeAlbumImages(images) {
  if (!images) return [];
  ...
  // Cover principale (priorité à cover_large > cover > thumbnail)
  if (images.cover_large) addImage(images.cover_large, 'cover_large');
  if (images.cover && images.cover !== images.cover_large) addImage(images.cover, 'cover');
  if (images.thumbnail && images.thumbnail !== images.cover) addImage(images.thumbnail, 'thumbnail');
  
  // Images additionnelles
  if (Array.isArray(images.all)) {
    images.all.forEach((img, i) => {
      const url = typeof img === 'string' ? img : img?.url || img?.uri;
      addImage(url, `image_${i + 1}`);
    });
  }
...
export function normalizeDiscogsReleaseDetail(data) {
  ...
  return {
    ...
    images: normalizeAlbumImages({
      ...
      cover: data.coverUrl || null,
      cover_large: data.coverUrlLarge || null,
      all: (data.images || []).map(img => img.uri || img.url)
    }),
