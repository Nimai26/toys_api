...
/**
 * Normalise un objet ou tableau d'images vers le format [{url: "...", type?: "..."}]
 * Gère les formats: objet {primary, thumbnail, gallery}, tableau de strings, tableau d'objets
 * @param {Object|Array|string} images - Images source (plusieurs formats acceptés)
 * @param {Object} options - Options de normalisation
 * @param {string} options.primary - Clé de l'image principale dans l'objet source
 * @param {string} options.thumbnail - Clé du thumbnail dans l'objet source
 * @param {string} options.gallery - Clé de la galerie dans l'objet source
 * @returns {Array<{url: string, type?: string}>} Tableau d'images normalisées
 */
export function normalizeImagesToArray(images, options = {}) {
  if (!images) return [];
  ...
  if (Array.isArray(images)) {
    images.forEach((img, i) => {
      if (typeof img === 'string') {
        addImage(img, i === 0 ? 'primary' : null);
      } else if (img && typeof img === 'object') {
        addImage(img.url || img.image, img.type);
      }
    });
    return result;
  }
  
  // Si c'est un objet {primary, thumbnail, gallery, cover, etc.}
  if (typeof images === 'object') {
    // Images principales (ordre de priorité)
    const primaryKeys = ['primary', 'cover', 'main', 'box_front'];
    for (const key of primaryKeys) {
      if (images[key]) {
        addImage(images[key], 'primary');
        break;
      }
    }
    
    // Thumbnail
    if (images.thumbnail && images.thumbnail !== images.primary) {
      addImage(images.thumbnail, 'thumbnail');
    }
    
    // Box back
    if (images.box_back) {
      addImage(images.box_back, 'box_back');
    }
    
    // Background
    if (images.background) {
      addImage(images.background, 'background');
    }
    
    // Galerie / screenshots / artworks
    const galleryKeys = ['gallery', 'screenshots', 'artworks', 'all'];
      ...
      if (Array.isArray(images[key])) {
        images[key].forEach(img => {
          if (typeof img === 'string') {
            ...
            addImage(img.url, img.type);
          }
        });
      }
    }
    
    // Autres URLs spécifiques
    if (images.cover_small) addImage(images.cover_small, 'cover_small');
    if (images.cover_large) addImage(images.cover_large, 'cover_large');
  }
...
/**
 * Normalise les images Mega en tableau [{url: "..."}] sans doublons
 * Priorité aux images haute qualité (sans suffixe de taille comme _600x600)
 * @param {Array<string>} images - Tableau d'URLs d'images
 * @param {string} thumbnail - URL du thumbnail
 * @returns {Array<{url: string}>} Tableau d'images normalisées
 */
export function normalizeMegaImages(images, thumbnail) {
  ...
  if (Array.isArray(images)) {
    images.forEach(url => {
      if (typeof url === 'string' && url) allUrls.add(url);
    });
  }
  
  // Ajouter le thumbnail s'il n'est pas déjà présent
  if (thumbnail && typeof thumbnail === 'string') {
    allUrls.add(thumbnail);
  }
  
  // Grouper les images par base (sans suffixe de taille)
  // Exemple: image_600x600.jpg et image.jpg sont la même image
  const imageGroups = new Map();
  
  allUrls.forEach(url => {
    // Extraire la base de l'URL sans le suffixe de taille Shopify (_600x600, _1200x1200, etc.)
    const baseMatch = url.match(/^(.+?)(_\d+x\d+)?(\.[^.?]+)(\?.*)?$/);
    if (baseMatch) {
      const [, base, size, ext, query] = baseMatch;
      const key = base + ext; // Clé sans taille ni query
      
      // Priorité: pas de suffixe > grand suffixe > petit suffixe
      let priority = 0;
      if (!size) {
        priority = 100; // Pas de suffixe = meilleure qualité
      } else {
        // Extraire la taille (ex: 600x600)
        const sizeMatch = size.match(/_(\d+)x(\d+)/);
        if (sizeMatch) {
          priority = parseInt(sizeMatch[1], 10); // Plus grand = meilleure qualité
        }
      }
  ...
  // Retourner les meilleures images sans doublons, triées par priorité décroissante
...
export function normalizeLegoDetail(raw, lang = 'fr-FR') {
  ...
  // Images - normaliser en tableau [{url, type}]
  const primaryImg = raw.baseImgUrl || (raw.images && raw.images[0]);
  const thumbImg = raw.thumb || raw.primaryImage || primaryImg;
  result.images = normalizeImagesToArray({
    primary: primaryImg,
    thumbnail: thumbImg,
    gallery: Array.isArray(raw.images) ? raw.images.slice(1) : []
  });
...
export function normalizePlaymobilDetail(raw, lang = 'fr-FR') {
  ...
  if (Array.isArray(raw.images)) {
    for (const img of raw.images) {
      if (img.includes('_box_front')) boxFront = img;
      if (img.includes('_box_back')) boxBack = img;
    }
  }
  result.images = normalizeImagesToArray({
    primary: raw.baseImgUrl,
    thumbnail: raw.thumb,
    box_front: boxFront,
    box_back: boxBack,
    gallery: Array.isArray(raw.images) ? raw.images : []
  });
...
export function normalizeKlickypediaDetail(raw, lang = 'fr') {
  ...
  // Marque et thème
  result.brand = 'Playmobil';
  result.theme = raw.attributes?.theme || raw.theme;
  
  // Année (depuis availability ou directement)
  result.year = raw.availability?.released || raw.released || null;
  
  // Specs
  result.specs.figureCount = raw.attributes?.figureCount || raw.figureCount;
  
  // Packaging
  result.packaging.format = raw.attributes?.format || raw.format;
  ...
  // Instructions de montage (fallback depuis Playmobil)
  if (raw.instructions && raw.instructions.available) {
    result.instructions = {
      available: true,
      productId: raw.instructions.productId,
      url: raw.instructions.url,
      format: raw.instructions.format || 'PDF',
      source: raw.instructions.source || 'playmobil',
      count: 1,
      manuals: [{
        url: raw.instructions.url,
        format: raw.instructions.format || 'PDF'
...
export function normalizeMegaDetail(raw, lang = 'fr-FR') {
  ...
  // Images - convertir en tableau normalisé [{url: "..."}] sans doublons
  // Priorité aux images haute qualité (sans suffixe de taille)
  result.images = normalizeMegaImages(raw.images, raw.thumbnail);
...
export function normalizeRebrickableDetail(raw) {
  ...
  // Images - normaliser en tableau [{url, type}]
  result.images = normalizeImagesToArray({
    primary: raw.set_img_url,
    thumbnail: raw.set_img_url
  });
