...
export function normalizePaninimiaAlbumDetail(rawData) {
  ...
  if (rawData.checklist) {
    ...
    // Ajouter les stickers spéciaux de la checklist (lettres comme A, B, C)
    if (rawData.checklist.special && Array.isArray(rawData.checklist.special) && rawData.checklist.special.length > 0) {
      checklist.special = rawData.checklist.special;
    }
