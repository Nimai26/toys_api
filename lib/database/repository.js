...
export async function searchLocal(searchQuery = '', options = {}) {
  ...
  try {
    ...
    // Filtre par texte (si fourni)
    if (searchQuery && searchQuery.trim()) {
      sql += ` AND name_search LIKE '%' || normalize_text($${paramIndex}) || '%'`;
    ...
    // Tri : correspondance exacte d'abord, puis popularité, puis année
    if (searchQuery && searchQuery.trim()) {
      sql += ` ORDER BY 
    ...
    log.debug(`searchLocal: "${searchQuery}" (source=${source}, type=${type})`);
...
export async function saveSearchResults(provider, searchType, searchQuery, params, results) {
  ...
  try {
    ...
    await query(
      `INSERT INTO searches (query, provider, search_type, result_ids, result_count, total_results, expires_at, cached_results)
      ...
      [cacheKey, provider, searchType, resultIds, resultIds.length, totalResults, expiresAt, JSON.stringify(results)]
    );
 ...
 */
export async function getCachedSearch(provider, searchType, searchQuery, params = {}) {
  if (!isCacheEnabled()) return null;
  ...
  try {
    ...
    const result = await queryOne(
      `SELECT cached_results, result_ids, total_results FROM searches
       WHERE query = $1 AND provider = $2 AND search_type = $3
       AND (expires_at IS NULL OR expires_at > NOW())`,
      [cacheKey, provider, searchType]
    );

    if (result && result.cached_results) {
      log.debug(`Search cache HIT: ${provider}/${searchType} "${searchQuery}"`);
      return typeof result.cached_results === 'string' 
        ? JSON.parse(result.cached_results) 
