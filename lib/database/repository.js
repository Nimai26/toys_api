 ...
 * @param {string} [options.entryType] - Type d'entrée requis ('search' ou 'detail')
 * @returns {Promise<object|null>}
 */
export async function getItem(source, sourceId, options = {}) {
  if (!isCacheEnabled()) return null;

  try {
    const id = generateItemId(source, sourceId);
    const requiredEntryType = options?.entryType || null;
    
    // Construire la requête avec filtre optionnel sur entry_type
    let queryStr = `SELECT * FROM items 
       WHERE id = $1 
       AND (expires_at IS NULL OR expires_at > NOW())`;
    const params = [id];
    
    if (requiredEntryType) {
      queryStr += ` AND entry_type = $2`;
      params.push(requiredEntryType);
    }
    
    const item = await queryOne(queryStr, params);
    ...
    if (item) {
      ...
      log.debug(`Cache HIT: ${id}${requiredEntryType ? ` (type: ${requiredEntryType})` : ''}`);
      return item.data;
    }

    log.debug(`Cache MISS: ${id}${requiredEntryType ? ` (type requis: ${requiredEntryType})` : ''}`);
    return null;
  } catch (err) {
...
export async function saveItem(source, sourceId, type, name, data, options = {}) {
  ...
  try {
    ...
    const entryType = options?.entryType || 'search'; // 'search' ou 'detail'
    ...
    await query(
      ...
      ON CONFLICT (id) DO UPDATE SET
        ...
        entry_type = CASE 
          WHEN EXCLUDED.entry_type = 'detail' THEN 'detail'
          ELSE items.entry_type
        END,
        year = EXCLUDED.year,
        ...
        data = CASE 
          WHEN EXCLUDED.entry_type = 'detail' OR pg_column_size(EXCLUDED.data) > pg_column_size(items.data) 
          THEN EXCLUDED.data 
          ELSE items.data 
        END,
