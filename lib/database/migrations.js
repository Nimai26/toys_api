...
const MIGRATIONS = {
  ...
  3: {
    description: 'Recherche par similarité - ajout search_term et index trigram',
    up: async (client) => {
      // Ajouter la colonne search_term pour stocker le terme de recherche brut
      await client.query(`
        ALTER TABLE searches 
        ADD COLUMN IF NOT EXISTS search_term TEXT
      `);
      
      // Colonne normalisée pour la similarité (sans accents, en minuscules)
      await client.query(`
        ALTER TABLE searches 
        ADD COLUMN IF NOT EXISTS search_term_normalized TEXT 
        GENERATED ALWAYS AS (normalize_text(search_term)) STORED
      `);
      
      // Index GIN trigram pour recherche par similarité rapide
      await client.query(`
        CREATE INDEX IF NOT EXISTS idx_searches_term_trgm 
        ON searches USING GIN(search_term_normalized gin_trgm_ops)
      `);
      
      // Index standard pour recherches exactes
      await client.query(`
        CREATE INDEX IF NOT EXISTS idx_searches_term 
        ON searches(search_term_normalized)
      `);
      
      // Mettre à jour les lignes existantes en extrayant le terme de recherche du JSON
      await client.query(`
        UPDATE searches 
        SET search_term = COALESCE(
          (query::jsonb)->>'query',
          (query::jsonb)->>'q',
          query
        )
      ...
      // Configurer le seuil de similarité par défaut (0.3 = 30% de correspondance)
      await client.query(`SELECT set_limit(0.3)`);
      
      log.info('📊 Colonne search_term et index trigram ajoutés pour la recherche par similarité');
    }
  },
  
  4: {
    description: 'Ajout entry_type pour différencier recherches et détails',
    up: async (client) => {
      // Ajouter la colonne entry_type pour distinguer 'search' vs 'detail'
      await client.query(`
        ALTER TABLE items 
        ADD COLUMN IF NOT EXISTS entry_type TEXT DEFAULT 'search'
      `);
      
      // Index pour requêtes filtrant par entry_type
      await client.query(`
        CREATE INDEX IF NOT EXISTS idx_items_entry_type 
        ON items(entry_type)
      `);
      
      // Mettre à jour les entrées existantes - par défaut tout est 'search'
      // Les détails seront marqués lors de leur prochain fetch
      await client.query(`
        UPDATE items 
        SET entry_type = 'search'
        WHERE entry_type IS NULL
      `);
      
      log.info('📊 Colonne entry_type ajoutée à la table items pour différencier recherches et détails');
    }
  }
};
