...
export async function withCache({ 
  ...
  // IMPORTANT: Pour les détails, on ne veut QUE les entrées de type 'detail'
  // Les entrées 'search' ont des données incomplètes
  if (!forceRefresh) {
    try {
      // Utiliser getItem avec filtre entry_type='detail' pour ne récupérer que les données complètes
      const cached = await getItem(source, externalId, { entryType: 'detail' });
      if (cached) {
        log.debug(`[${source}] Cache HIT (detail)`, { externalId });
  ...
  // Sauvegarder en base (async, non-bloquant) - marqué comme 'detail'
  if (isDatabaseConnected() && normalizedData && typeof normalizedData === 'object') {
    saveItem(source, externalId, type, itemName, normalizedData, { entryType: 'detail' })
      .then(success => {
        const duration = Date.now() - startTime;
        if (success) {
          log.debug(`[${source}] Sauvegardé en cache PostgreSQL (detail)`, { externalId, duration: `${duration}ms` });
        } else {
...
export async function withSearchCache({
  ...
  // Vérifier le cache de recherche (sauf si forceRefresh)
  if (CACHE_MODE !== 'db_only' && !forceRefresh) {
    try {
    ...
    log.debug(`[${source}] Cache bypassed (forceRefresh=true)`, { query });
  }
  ...
  if (isDatabaseConnected()) {
    ...
    // NOTE: Ces items sont marqués 'search' car ils contiennent des données minimales
    // Les appels de détail créeront des entrées 'detail' avec données complètes
    if (cacheResults && normalizedResults.results && Array.isArray(normalizedResults.results)) {
      for (const item of normalizedResults.results) {
        if (item.id || item.external_id || item.sourceId) {
          ...
          saveItem(source, String(itemId), type, itemName, item, { entryType: 'search' })
...
export function createProviderCache(source, defaultType) {
  return {
     ...
     * @param {string} query - Requête de recherche (ou objet JSON pour clé de cache)
     * @param {Function} fetchFn - Fonction async pour appeler l'API
     * @param {Object} options - Options
     * @param {boolean} options.forceRefresh - Ignorer le cache (refresh=true, cache=false, db=false)
     */
