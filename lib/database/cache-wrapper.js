...
export async function withSearchCache({
  ...
  if (!isCacheEnabled() || CACHE_MODE === 'api_only') {
    ...
    return normalizeResultsFn ? normalizeResultsFn(results) : results;
  }
  
  // Vérifier le cache de recherche (courte durée)
  if (CACHE_MODE !== 'db_only') {
    try {
  ...
  const duration = Date.now() - startTime;
  log.debug(`[${source}] Recherche terminée`, { query, duration: `${duration}ms`, resultsCount: normalizedResults.results?.length });
