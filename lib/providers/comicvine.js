 ...
 */
export async function getComicVineVolume(volumeId, options = {}) {
  metrics.sources.comicvine.requests++;
  ...
  try {
    ...
    // Synopsis original
    const synopsisOriginal = item.deck || null;
 ...
 */
export async function getComicVineIssue(issueId, options = {}) {
  metrics.sources.comicvine.requests++;
 ...
 */
export async function getComicVineVolumeNormalized(volumeId, options = {}) {
  const result = await getComicVineVolume(volumeId, options);
 ...
 * @returns {Promise<object>} - Détails normalisés
 */
export async function getComicVineIssueNormalized(issueId, options = {}) {
  const result = await getComicVineIssue(issueId, options);
