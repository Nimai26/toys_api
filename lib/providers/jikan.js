...
async function fetchAnimeAdditionalData(animeId) {
  ...
  try {
    ...
    const charsResponse = await fetch(`${JIKAN_BASE_URL}/anime/${animeId}/characters`, {
      headers: { 'Accept': 'application/json' }
    });
    if (charsResponse.ok) {
    ...
    // Staff
    const staffResponse = await fetch(`${JIKAN_BASE_URL}/anime/${animeId}/staff`, {
      headers: { 'Accept': 'application/json' }
    });
    if (staffResponse.ok) {
    ...
    // Recommendations
    const recsResponse = await fetch(`${JIKAN_BASE_URL}/anime/${animeId}/recommendations`, {
      headers: { 'Accept': 'application/json' }
    });
    if (recsResponse.ok) {
...
async function fetchMangaAdditionalData(mangaId) {
  ...
  try {
    // Characters
    const charsResponse = await fetch(`${JIKAN_BASE_URL}/manga/${mangaId}/characters`, {
      headers: { 'Accept': 'application/json' }
    });
    if (charsResponse.ok) {
      const charsData = await charsResponse.json();
    ...
    // Recommendations
    const recsResponse = await fetch(`${JIKAN_BASE_URL}/manga/${mangaId}/recommendations`, {
      headers: { 'Accept': 'application/json' }
    });
    if (recsResponse.ok) {
      const recsData = await recsResponse.json();
