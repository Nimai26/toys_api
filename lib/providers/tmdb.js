...
export async function getTmdbMovieById(id, apiKey, options = {}) {
  ...
  try {
    ...
    if (shouldTranslate && destLang) {
      // Traduit l'overview
      if (movie.overview) {
        const result = await translateText(movie.overview, destLang, { enabled: true });
        if (result.translated) {
          finalOverview = result.text;
          overviewTranslated = result.text;
        }
      }
...
export async function getTmdbTvById(id, apiKey, options = {}) {
  ...
  try {
    ...
    if (shouldTranslate && destLang) {
      // Traduit l'overview
      if (tv.overview) {
        const result = await translateText(tv.overview, destLang, { enabled: true });
        if (result.translated) {
          finalOverview = result.text;
          overviewTranslated = result.text;
        }
...
/**
 * Recherche TMDB avec résultats normalisés (films)
 * @param {string} query - Terme de recherche
 * @param {string} apiKey - Clé API TMDB
 * @param {object} options - Options de recherche
 * @returns {Promise<object>} - Résultats normalisés
 */
export async function searchTmdbMovieNormalized(query, apiKey, options = {}) {
  const result = await searchTmdb(query, apiKey, { ...options, type: 'movie' });
  return normalizeTmdbMovieSearch(result);
}

/**
 * Recherche TMDB avec résultats normalisés (séries)
 * @param {string} query - Terme de recherche
 * @param {string} apiKey - Clé API TMDB
 * @param {object} options - Options de recherche
 * @returns {Promise<object>} - Résultats normalisés
 */
export async function searchTmdbSeriesNormalized(query, apiKey, options = {}) {
  const result = await searchTmdb(query, apiKey, { ...options, type: 'tv' });
  return normalizeTmdbSeriesSearch(result);
}

/**
 * Détails film TMDB normalisés
 * @param {string|number} id - ID du film
 * @param {string} apiKey - Clé API TMDB
 * @param {object} options - Options
 * @returns {Promise<object>} - Détails normalisés
 */
export async function getTmdbMovieByIdNormalized(id, apiKey, options = {}) {
  const result = await getTmdbMovieById(id, apiKey, options);
  return normalizeTmdbMovieDetail(result);
}

/**
 * Détails série TMDB normalisés
 * @param {string|number} id - ID de la série
 * @param {string} apiKey - Clé API TMDB
 * @param {object} options - Options
 * @returns {Promise<object>} - Détails normalisés
 */
export async function getTmdbTvByIdNormalized(id, apiKey, options = {}) {
  const result = await getTmdbTvById(id, apiKey, options);
  return normalizeTmdbSeriesDetail(result);
}
