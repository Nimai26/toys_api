...
export async function getTransformerlandItemDetails(itemId, retries = MAX_RETRIES, options = {}) {
  ...
  while (attempt < retries) {
    ...
    try {
      ...
      const pageSolution = await fsrRequest("request.get", itemUrl, null, {
        waitInSeconds: 2
      }, 45000);
      ...
      if (shouldTranslate && destLang) {
        ...
        if (nameOriginal) {
          const translatedName = await translateText(nameOriginal, destLang);
          if (translatedName !== nameOriginal) {
