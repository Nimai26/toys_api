...
/**
 * Recherche de livres par auteur sur Google Books API
 ...
 */
export async function searchGoogleBooksByAuthor(author, apiKey, options = {}) {
  // Utilise le paramètre inauthor: pour rechercher par auteur
  const query = `inauthor:"${author}"`;
  return searchGoogleBooks(query, apiKey, options);
}
...
export async function getGoogleBookById(volumeId, apiKey, options = {}) {
  ...
  try {
    ...
    // Prendre uniquement la meilleure qualité disponible (les différents zooms sont la même image)
    const bestCover = covers.extraLarge || covers.large || covers.medium || covers.small || covers.thumbnail || null;
    const imageArray = bestCover ? [bestCover] : [];
