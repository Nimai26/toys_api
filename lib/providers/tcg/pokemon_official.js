...
export async function searchPokemonCardsOfficial(query, options = {}) {
  ...
  if (!forceRefresh) {
    ...
    logger.debug(`[Pokemon Official] Cache bypassed (forceRefresh=true)`, { query });
  }
  ...
  try {
    ...
    while (allResults.length < max && currentPage <= maxPages) {
      ...
      const { html, status } = await puppeteerStealth.stealthGet(pageUrl, {
        ...
        fullPageScroll: true // Scroller jusqu'en bas pour charger toutes les cartes (lazy loading)
      });
...
export async function getPokemonCardDetailsOfficial(set, number, options = {}) {
  ...
  if (!forceRefresh) {
    ...
    logger.debug(`[Pokemon Official] Cache bypassed (forceRefresh=true)`, { set, number });
  }
