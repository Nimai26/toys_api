...
export async function searchYuGiOhCards(query, options = {}) {
  ...
  logger.info(`[Yu-Gi-Oh!] Searching: ${query} (max ${max})`);
  
  try {
    // Vérifier le cache
    const cacheKey = `yugioh_search_${query}_${type || 'all'}_${race || 'all'}_${max}`;
    if (getCached) {
...
export async function getYuGiOhCardDetails(cardId, options = {}) {
  ...
  logger.info(`[Yu-Gi-Oh!] Fetching card: ${cardId}`);
  
  try {
    // Vérifier le cache
    const cacheKey = `yugioh_card_${cardId}`;
    if (getCached) {
      const cached = await getCached(cacheKey);
      if (cached) {
        logger.debug(`[Yu-Gi-Oh!] Cache hit: ${cacheKey}`);
    ...
    const data = await ygoprodeckRequest('/cardinfo.php', { id: cardId });
...
export async function getYuGiOhCardByName(name, options = {}) {
  ...
  logger.info(`[Yu-Gi-Oh!] Fetching card by name: ${name}`);
  
  try {
    // Vérifier le cache
    const cacheKey = `yugioh_name_${name}`;
    if (getCached) {
      const cached = await getCached(cacheKey);
      if (cached) {
        logger.debug(`[Yu-Gi-Oh!] Cache hit: ${cacheKey}`);
    ...
    const data = await ygoprodeckRequest('/cardinfo.php', { name: name });
