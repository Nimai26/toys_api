  ...
  while (attempt < retries) {
    ...
    try {
      ...
      if (checklistMatch) {
        ...
        // Parser la checklist qui peut contenir des chiffres ET des lettres
        // Ex: "1 à 240, A à X" ou "1 à 100, I, II, III"
        const parts = checklistRaw.split(/[,;]/);
        
        for (const part of parts) {
          const trimmed = part.trim();
          
          // Tenter de parser comme numéros
          ...
          if (numRangeMatch) {
            ...
            // Si ce n'est pas un nombre, c'est peut-être une lettre ou alphanumérique
            const letterRangeMatch = trimmed.match(/^([A-Z])\s*(?:à|-)\s*([A-Z])$/i);
            if (letterRangeMatch) {
              // Range de lettres : A à X
              const start = letterRangeMatch[1].toUpperCase().charCodeAt(0);
              const end = letterRangeMatch[2].toUpperCase().charCodeAt(0);
              for (let i = start; i <= end; i++) {
                checklistSpecials.push(String.fromCharCode(i));
              }
            } else if (/^[A-Z]+\d*$/i.test(trimmed)) {
              // Lettre seule ou alphanumérique : A, B, C ou A1, B2
              checklistSpecials.push(trimmed.toUpperCase());
            }
          }
        }
      }
      ...
      const checklist = checklistRaw ? {
        ...
        special: checklistSpecials.length > 0 ? checklistSpecials : undefined
      } : null;
      ...
      if (checklist) {
        ...
        // Compter les images spéciales depuis la checklist
        if (checklist.special && Array.isArray(checklist.special)) {
          totalSpecials += checklist.special.length;
        }
        
        // Ajouter les images spéciales depuis specialStickers (si différentes)
        if (specialStickers && specialStickers.length > 0) {
          for (const special of specialStickers) {
            // Éviter double comptage : si les items sont déjà dans checklist.special
            const itemsInChecklist = checklist.special || [];
            const uniqueItems = special.list.filter(item => !itemsInChecklist.includes(item));
            totalSpecials += uniqueItems.length;
          }
        }
