 ...
 * @param {string} destLang - Langue de destination (ex: "fr", "de", "es")
 * @returns {Promise<string>} - Texte traduit ou original en cas d'erreur
 */
async function translateText(text, destLang) {
  if (!text || !AUTO_TRAD_ENABLED) return text;
  
  // Ne pas traduire si déjà en anglais demandé
  if (destLang === 'en') return text;
  ...
  try {
    ...
    if (!response.ok) {
      log.warn(`Erreur traduction: ${response.status}`);
      return text;
    }
    
    const data = await response.json();
    if (data.results && data.results[0] && data.results[0].translated) {
      log.debug(`Plot traduit en ${destLang}`);
      return data.results[0].translated;
    }
    return text;
  } catch (err) {
 ...
 * @param {object} options - Options (lang: langue pour le plot)
 * @returns {Promise<object>} - Détails du titre
 */
export async function getImdbTitleById(titleId, options = {}) {
  const { lang = 'en' } = options;
  const destLang = lang.split('-')[0]; // "fr-FR" -> "fr"
  
  // Cache inclut la langue pour les traductions
  const cacheKey = `imdb_title_${titleId}_${destLang}`;
  const cached = getCached(cacheKey);
  if (cached) {
    log.debug(`Cache hit: title ${titleId} (${destLang})`);
    metrics.requests.cached++;
    return cached;
  }
  
  log.debug(`Récupération titre: ${titleId} (lang: ${destLang})`);
  metrics.sources.imdb.requests++;
  ...
  try {
    ...
    const result = {
      ...
      // Plot traduit si langue != en
      plot: title.plot ? await translateText(title.plot, destLang) : null,
      plotOriginal: title.plot || null,
      plotLanguage: destLang,
      genres: title.genres || [],
