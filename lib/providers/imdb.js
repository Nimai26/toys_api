 ...
 * @param {boolean} enabled - Si la traduction est activée (paramètre autoTrad)
 * @returns {Promise<string>} - Texte traduit ou original en cas d'erreur
 */
async function translateText(text, destLang, enabled = false) {
  // Ne traduit QUE si autoTrad=1 est passé explicitement
  if (!text || !enabled || !AUTO_TRAD_ENABLED) return text;
...
export async function getImdbTitleById(titleId, options = {}) {
  ...
  try {
    ...
    const result = {
      ...
      plot: title.plot ? await translateText(title.plot, destLang, shouldTranslate) : null,
      plotOriginal: shouldTranslate ? (title.plot || null) : undefined,
      plotTranslated: shouldTranslate,
      genres: title.genres || [],
