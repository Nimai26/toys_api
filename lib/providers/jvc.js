...
export async function searchJVC(query, options = {}) {
  ...
  try {
    ...
    // Nouveau regex pour la structure rechercher.php
    // Structure: <a href="/jeux/jeu-XXXX/" class="... stretched-link ...">Titre</a>
    const gameRegex = /href="\/jeux\/jeu-(\d+)\/"[^>]*class="[^"]*stretched-link[^"]*"[^>]*>\s*([^<]+)\s*<\/a>/gi;
    
    // Regex pour les plateformes (dans le même bloc card)
    const platformRegex = /<span[^>]*class="[^"]*card__plateform[^"]*"[^>]*>([^<]+)<\/span>/gi;
    
    // Regex pour les images
    const imgRegex = /data-srcset="([^"]+)"/gi;
    ...
    while ((match = imgRegex.exec(html)) !== null) {
      ...
      if (firstUrl && firstUrl.includes('jeuxvideo.com')) {
