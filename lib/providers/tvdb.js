...
export async function getTvdbSeriesById(id, apiKey, options = {}) {
  ...
  const cacheKey = `tvdb_series_${id}_${lang}_${shouldTranslate ? 'trad' : 'notrad'}`;
  const cached = getCached(cacheKey);
  if (cached) {
  ...
  log.debug(`Récupération série: ${id} (lang: ${lang}, autoTrad: ${shouldTranslate})`);
  metrics.sources.tvdb.requests++;
  ...
  try {
    ...
    // Prépare les genres
    const genresList = series.genres?.map(g => g.name) || [];
    ...
    if (shouldTranslate && destLang) {
      ...
      if (series.overview && !translations?.overview) {
        const translatedOverview = await translateText(series.overview, destLang);
    ...
    const result = {
      ...
      genresFull: series.genres?.map(g => ({
        id: g.id,
        name: g.name,
        slug: g.slug
      })) || [],
...
export async function getTvdbMovieById(id, apiKey, options = {}) {
  ...
  const cacheKey = `tvdb_movie_${id}_${lang}_${shouldTranslate ? 'trad' : 'notrad'}`;
  const cached = getCached(cacheKey);
  if (cached) {
  ...
  try {
    ...
    // Applique la traduction si demandée
    let finalOverview = translations?.overview || movie.overview;
    ...
    if (shouldTranslate && destLang) {
      // Traduit l'overview si pas déjà traduit par TVDB
      if (movie.overview && !translations?.overview) {
