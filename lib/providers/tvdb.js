...
export async function getTvdbSeriesById(id, apiKey, options = {}) {
  ...
  try {
    ...
    // meta=people pour inclure toute l'équipe (directors, writers, creators, etc.)
    const url = `${TVDB_BASE_URL}/series/${id}/extended?meta=people`;
...
export async function getTvdbMovieById(id, apiKey, options = {}) {
  ...
  if (cached) {
    ...
    console.log('[DEBUG TVDB] Returned from INTERNAL cache, overview:', cached.overview);
    metrics.requests.cached++;
    return cached;
  }
  ...
  try {
    ...
    if (movie.overviewTranslations && movie.overviewTranslations.includes('eng')) {
      try {
        const engResponse = await fetch(`${TVDB_BASE_URL}/movies/${id}/translations/eng`, {
          headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' }
        });
        if (engResponse.ok) {
    ...
    if (shouldTranslate && destLang) {
      ...
      if (baseOverview && !translations?.overview) {
        const translatedOverview = await translateText(baseOverview, destLang);
        if (translatedOverview !== baseOverview) {
          finalOverview = translatedOverview;
          overviewTranslated = translatedOverview;
        }
      } else if (translations?.overview) {
