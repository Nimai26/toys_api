...
export async function searchBedethequeByAuthor(author, options = {}) {
  ...
  try {
    ...
    const authorName = authorData.label || authorData.value;
    
    log.debug(`Auteur trouvé: ${authorName} (ID: ${authorId})`);
    
    // Étape 2: Récupérer la page de l'auteur avec FlareSolverr pour obtenir ses séries
    const authorUrl = `${BEDETHEQUE_BASE_URL}/auteur-${authorId}-.html`;
    log.debug(`Récupération page auteur: ${authorUrl}`);
    ...
    // Étape 3: Parser les séries de l'auteur
    const seriesRegex = /<li>\s*<a[^>]*href="\/serie-(\d+)-[^"]*"[^>]*>([^<]+)<\/a>/gi;
    const seriesSet = new Set();
    const seriesData = [];
    let serieMatch;
    
    while ((serieMatch = seriesRegex.exec(html)) !== null) {
      const [, serieId, serieName] = serieMatch;
      if (!seriesSet.has(serieId)) {
        seriesSet.add(serieId);
        seriesData.push({
          id: parseInt(serieId),
          name: decodeHtmlEntities(serieName.trim())
        });
      }
    }
    ...
    // Étape 4: Récupérer les albums de chaque série (limité par max)
    const allAlbums = [];
    
    for (const serie of seriesData) {
      if (allAlbums.length >= max) break;
      
      const slug = serie.name
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
        .replace(/[^\w\s-]/g, '')
        .replace(/\s+/g, '-');
      
      const albumsUrl = `${BEDETHEQUE_BASE_URL}/albums-${serie.id}-BD-${slug}.html`;
