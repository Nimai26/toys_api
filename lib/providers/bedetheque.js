...
export async function searchBedetheque(query, options = {}) {
  ...
  try {
    ...
    log.debug(`✅ ${results.length} séries trouvées`);
...
/**
 * Récupère les détails d'une série Bedetheque normalisés
 * @param {string|number} serieId - ID de la série
 * @returns {Promise<object>} - Détails normalisés
 */
export async function getBedethequeSerieByIdNormalized(serieId) {
  const result = await getBedethequeSerieById(serieId);
  return normalizeBedethequeSerieDetail(result);
}

/**
 * Récupère les détails d'un album Bedetheque normalisés
 * @param {string|number} albumId - ID de l'album
 * @returns {Promise<object>} - Détails normalisés
 */
export async function getBedethequeAlbumByIdNormalized(albumId) {
  const result = await getBedethequeAlbumById(albumId);
  return normalizeBedethequeAlbumDetail(result);
}
