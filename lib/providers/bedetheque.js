 ...
 * Utilise l'API AJAX pour récupérer l'ID de l'auteur, puis scrape la page auteur pour obtenir les séries
 * 
...
export async function searchBedethequeByAuthor(author, options = {}) {
  ...
  try {
    ...
    // 1. Récupérer l'ID de l'auteur via l'API AJAX
    const ajaxUrl = `${BEDETHEQUE_BASE_URL}/ajax/auteurs?term=${encodeURIComponent(author)}`;
    log.debug(`Requête AJAX: ${ajaxUrl}`);
    ...
    const authors = await ajaxResponse.json();
    // Format: [{"id": "2220", "label": "Goscinny, René", "value": "Goscinny, René"}]
    ...
    if (!authors || authors.length === 0) {
      ...
      return {
        ...
        albums: [],
        source: 'bedetheque',
        note: 'Auteur non trouvé dans la base Bedetheque'
      };
    }
    
    // Si plusieurs auteurs, prioriser par pertinence
    const normalizedAuthor = author.toLowerCase().trim();
    let authorData = authors.find(a => a.value.toLowerCase() === normalizedAuthor);
    
    // Cas spécial: si on cherche juste "Goscinny", prendre René en priorité
    if (!authorData && authors.length > 1 && normalizedAuthor === 'goscinny') {
      authorData = authors.find(a => a.value.toLowerCase().includes('rené'));
    }
    
    if (!authorData) {
      authorData = authors.find(a => a.value.toLowerCase().startsWith(normalizedAuthor));
    }
    if (!authorData) {
    ...
    const authorName = authorData.value;
    
    log.debug(`Auteur trouvé: ${authorName} (ID: ${authorId})`);
    
    // 2. Construire l'URL de la page auteur
    // Format: /auteur-{id}-BD-{Nom-Prenom}.html
    const authorSlug = authorName
      .replace(/,\s*/g, '-')  // "Goscinny, René" -> "Goscinny-René"
      .normalize('NFD')
    ...
    const authorUrl = `${BEDETHEQUE_BASE_URL}/auteur-${authorId}-BD-${authorSlug}.html`;
    log.debug(`URL page auteur: ${authorUrl}`);
    
    // 3. Récupérer la page auteur via FlareSolverr
    ...
    // 4. Parser les séries de cet auteur avec filtrage
    const seriesData = [];
    const seenSerieIds = new Set();
    
    // Regex pour extraire les liens série
    const serieRegex = /href="[^"]*\/serie-(\d+)-BD-([^"]+)\.html"/gi;
    let match;
    
    while ((match = serieRegex.exec(html)) !== null) {
      const serieId = match[1];
      const serieSlug = match[2];
      
      if (seenSerieIds.has(serieId)) continue;
      seenSerieIds.add(serieId);
      
      // Décoder le nom de la série depuis le slug
      const serieName = decodeURIComponent(serieSlug).replace(/-/g, ' ').trim();
      
      // FILTRAGE 1: Exclure les catalogues, expositions, recueils, ventes aux enchères
      const isExcluded = /catalogues?|expositions?|ventes aux ench[eè]res|recueils?/i.test(serieName);
      if (isExcluded) {
        log.debug(`  Série exclue (catalogue/exposition): ${serieName}`);
        continue;
      }
      ...
      seriesData.push({
        id: parseInt(serieId),
        name: serieName,
        url: `${BEDETHEQUE_BASE_URL}/serie-${serieId}-BD-${serieSlug}.html`
      });
    }
    
    log.debug(`${seriesData.length} séries trouvées pour ${authorName}`);
    ...
    if (seriesData.length === 0) {
      return {
        ...
        albums: [],
        source: 'bedetheque',
        note: `Aucune série trouvée pour l'auteur ${authorName}`
      };
    }
    
    // 5. Récupérer les albums de chaque série (limiter au max demandé)
    const allAlbums = [];
    const seriesToFetch = seriesData.slice(0, Math.min(seriesData.length, 50)); // Max 50 séries (après filtrage)
    
    for (const serie of seriesToFetch) {
      if (allAlbums.length >= max) break;
      
      const albumsUrl = serie.url.replace('/serie-', '/albums-');
      log.debug(`Récupération albums série ${serie.id}: ${serie.name}`);
      
      const albumsResponse = await fetch(`${FSR_BASE}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          cmd: "request.get",
      ...
      // Parser les albums - même format que searchBedethequeAlbums
      const seenIds = new Set();
    ...
    const result = {
      ...
      albums: allAlbums,
      source: 'bedetheque',
      note: `${allAlbums.length} album(s) trouvé(s) dans ${seriesToFetch.length} série(s) pour ${authorName}`
    };

    log.debug(`✅ ${allAlbums.length} albums trouvés pour ${authorName} (${seriesData.length} séries)`);
    return result;

  } catch (err) {
    metrics.sources.bedetheque.errors++;
    log.error(`Erreur searchBedethequeByAuthor: ${err.message}`);
    throw err;
  }
}
