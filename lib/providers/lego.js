...
export async function getProductDetails(productId, lang = DEFAULT_LOCALE, retries = MAX_RETRIES) {
  ...
  while (attempt < retries) {
    ...
    try {
      ...
      // 1. D'abord chercher dans tous les JSON-LD (données structurées) - cherche un Product
      ...
      for (const jsonLdMatch of jsonLdMatches) {
        ...
        try {
          ...
          for (const item of items) {
            // Chercher un objet de type Product
            if (item['@type'] === 'Product' && item.name) {
              product.name = item.name.trim();
              log.debug("Nom extrait du JSON-LD Product:", product.name);
              break;
            }
          }
        } catch (e) {
      ...
      // 2. Chercher dans les données JSON embarquées (window.__APOLLO_STATE__, __NEXT_DATA__, etc.)
      if (!product.name) {
        const jsonNamePatterns = [
          /"productName"\s*:\s*"([^"]+)"/i,
          /"displayName"\s*:\s*"([^"]+)"/i,
          /"name"\s*:\s*"((?:LEGO[®]?\s+)?(?:Star Wars|City|Technic|Icons|Creator|Friends|Ninjago|Marvel|DC|Disney|Harry Potter|Ideas|Architecture|Speed Champions)[^"]+)"/i
        ];
      ...
      // Note: LEGO n'affiche pas toujours le nombre de minifigures sur la page produit
      // On privilégie les attributs data-test et les données JSON structurées
      const minifigPatterns = [
        /data-test="product-details-minifigures"[^>]*>([^<]+)</i,
        /"minifiguresCount"\s*:\s*(\d+)/i,
        /"minifigures"\s*:\s*(\d+)/i
        // Note: pattern générique "X figurines" retiré car il capture des valeurs incorrectes
        // (ex: "16 figurines" d'une boîte de présentation recommandée sur la page)
      ];
          ...
          for (const key of Object.keys(apolloState)) {
            if (key.startsWith("SingleVariantProduct:") || key.startsWith("Product:") || key.startsWith("MultiVariantProduct:")) {
              ...
              if (prodData && (prodData.productCode === productId || key.includes(productId))) {
                ...
                if (prodData.description && !product.description) {
                  // Nettoyer le HTML de la description
                  product.description = prodData.description.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
                }
                
                // Nouvelle structure Apollo: variant.id au lieu de variant.__ref
                const variantRef = prodData.variant?.__ref || prodData.variant?.id;
                if (variantRef) {
                  const variantData = apolloState[variantRef];
                  if (variantData) {
                    // Nouvelle structure: attributes est une référence vers $ProductVariant:xxx.attributes
                    const attrsRef = variantData.attributes?.__ref || variantData.attributes?.id;
                    const attrs = attrsRef ? apolloState[attrsRef] : variantData.attributes;
                    ...
                    if (attrs) {
                      ...
                      if (attrs.minifigureCount && !product.minifiguresCount) product.minifiguresCount = attrs.minifigureCount;
                      if (attrs.isNew !== undefined) product.isNew = attrs.isNew;
                      if (attrs.onSale !== undefined) product.onSale = attrs.onSale;
                    }
                    
                    // Nouvelle structure: price est une référence vers $ProductVariant:xxx.price
                    const priceRef = variantData.price?.__ref || variantData.price?.id;
                    if (priceRef) {
                      const priceData = apolloState[priceRef];
                      if (priceData?.formattedAmount && !product.price) {
                        product.price = priceData.formattedAmount;
                      }
                      if (priceData?.centAmount && !product.priceCents) {
                        product.priceCents = priceData.centAmount;
                      }
                      if (priceData?.currencyCode) {
                        product.currency = priceData.currencyCode;
                      }
                    } else if (variantData.price?.formattedAmount && !product.price) {
                      product.price = variantData.price.formattedAmount;
                    }
                    
                    // Liste price (prix barré)
                    const listPriceRef = variantData.listPrice?.__ref || variantData.listPrice?.id;
                    if (listPriceRef) {
                      const listPriceData = apolloState[listPriceRef];
                      if (listPriceData?.formattedAmount && !product.listPrice) {
                        product.listPrice = listPriceData.formattedAmount;
                      }
                    }
                  }
                }
    ...
    try {
      ...
      if (nextDataMatch) {
        try {
          ...
          // Fonction pour valider une URL de manuel
          const isValidManualUrl = (url) => {
            if (!url) return false;
            // Exclure les URLs qui se terminent par "product.bi.xxx.pdf" sans fichier après (c'est le dossier, pas un fichier)
            // Ex: ".../product.bi.additional.info.pdf" = invalide
            // Ex: ".../product.bi.additional.info.pdf/75192_X_Digital Booklet.pdf" = valide
          ...
          if (Array.isArray(buildingInstructions)) {
            for (const instr of buildingInstructions) {
              ...
              if (pdfUrl && isValidManualUrl(pdfUrl)) {
          ...
          for (const key of Object.keys(apolloState)) {
            ...
            if (key.includes("BuildingInstruction") || (data && data.pdfLocation)) {
              if (data.pdfLocation && isValidManualUrl(data.pdfLocation) && !instructions.manuals.find(m => m.pdfUrl === data.pdfLocation)) {
                instructions.manuals.push({
                  id: data.id || key,
        ...
        for (const pattern of pdfPatterns) {
          ...
          for (const m of matches) {
            ...
            if (!seenPdfs.has(pdfUrl)) {
              ...
              // Exclure les URLs qui se terminent par "product.bi.xxx.pdf" sans fichier après
              // Ex: ".../product.bi.additional.info.pdf" = invalide (c'est le dossier)
              // Ex: ".../product.bi.additional.info.pdf/75192_X_Digital Booklet.pdf" = valide
              const isGenericBiPdf = /product\.bi\.[^\/]+\.pdf\/?$/i.test(pdfUrl);
              // Doit se terminer par .pdf avec un vrai nom de fichier
              const isManual = !isGenericBiPdf && /\/[^\/]+\.pdf$/i.test(pdfUrl);
