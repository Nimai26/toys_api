  ...
  while (attempt < retries) {
    ...
    try {
      ...
      if (now - getLastLegoHomeVisit() > LEGO_SESSION_TTL) {
        ...
        const homeSolution = await fsrRequest("request.get", `https://www.lego.com/${lang.toLowerCase()}/`, {}, 30000);
      ...
      try {
        const graphqlSolution = await fsrRequest("request.post", GRAPHQL_URL, {
  ...
  while (attempt < retries) {
    ...
    try {
      ...
      if (now - getLastLegoHomeVisit() > LEGO_SESSION_TTL) {
        ...
        await fsrRequest("request.get", `https://www.lego.com/${lang.toLowerCase()}/`, {}, 30000);
        setLastLegoHomeVisit(now);
      }
      ...
      const pageSolution = await fsrRequest("request.get", productUrl, {
        waitInSeconds: 2
      }, 60000);
...
export async function getBuildingInstructions(productId, lang = DEFAULT_LOCALE, retries = MAX_RETRIES) {
  ...
  while (attempt < retries) {
    ...
    try {
      ...
      const pageSolution = await fsrRequest("request.get", instructionsUrl, {
        waitInSeconds: 2
      }, 60000);
