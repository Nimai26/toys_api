...
/**
 * Recherche Deezer avec résultat normalisé v3.0.0
 * @param {string} query - Terme de recherche
 * @param {Object} options - Options de recherche (limit, type)
 * @returns {Promise<Object>} Résultat normalisé
 */
export async function searchDeezerNormalized(query, options = {}) {
  const rawResult = await searchDeezer(query, { ...options, type: 'album' });
  return normalizeDeezerSearch(rawResult);
}

/**
 * Détails d'un album Deezer avec résultat normalisé v3.0.0
 * @param {number|string} albumId - ID album Deezer
 * @returns {Promise<Object>} Détail normalisé
 */
export async function getDeezerAlbumNormalized(albumId) {
  const rawResult = await getDeezerAlbum(albumId);
  return normalizeDeezerAlbumDetail(rawResult);
}
