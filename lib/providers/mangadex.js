 ...
 * Utilise l'endpoint /author pour trouver l'auteur, puis récupère ses mangas
...
export async function searchMangaDexByAuthor(author, options = {}) {
  ...
  try {
    log.debug(`Recherche auteur MangaDex: ${author}`);
    ...
    const authorUrl = `${MANGADEX_BASE_URL}/author?${authorParams}`;
    const authorResponse = await fetchViaProxy(authorUrl);
    
    if (!authorResponse.ok) {
      throw new Error(`Erreur API MangaDex /author: ${authorResponse.status}`);
    }
    ...
    // Prendre le premier auteur trouvé (meilleur match selon l'API MangaDex)
    const authorId = authorData.data[0].id;
    const authorName = authorData.data[0].attributes.name;
    log.debug(`Auteur trouvé: ${authorName} (${authorId})`);
    ...
    if (lang) {
      mangaParams.append('availableTranslatedLanguage[]', lang);
    }
    
    const mangaUrl = `${MANGADEX_BASE_URL}/manga?${mangaParams}`;
    const mangaResponse = await fetchViaProxy(mangaUrl);
    
    if (!mangaResponse.ok) {
      throw new Error(`Erreur API MangaDex /manga: ${mangaResponse.status}`);
    }
    ...
    return {
      query: author,
      type: 'author',
      totalResults: mangaData.total || results.length,
    ...
    log.error(`Erreur recherche auteur MangaDex: ${error.message}`);
    throw error;
  }
}
