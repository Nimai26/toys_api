...
// Utilise Puppeteer Stealth via proxy VPN (Gluetun)
// SÉCURITÉ: Tout le trafic Amazon passe par le VPN
...
} from '../normalizers/amazon.js';

// Import du module Puppeteer Stealth
let puppeteerStealth = null;
let puppeteerAvailable = false;

// Charger puppeteer-stealth de manière asynchrone
(async () => {
  try {
    puppeteerStealth = await import('../utils/puppeteer-stealth.js');
    puppeteerAvailable = true;
    log.info('✅ Puppeteer Stealth chargé - Mode anti-détection via VPN activé');
  } catch (err) {
    log.error('❌ Puppeteer Stealth non disponible - Amazon sera désactivé');
...
/**
 * Retourne le statut complet du provider Amazon
...
export function getAmazonStatus() {
  ...
  return {
    available: availability.available && puppeteerAvailable,
    reason: !puppeteerAvailable ? 'Puppeteer Stealth non disponible' : availability.reason,
    retryAfter: availability.retryAfter,
    puppeteer: {
      available: puppeteerAvailable,
      stats: puppeteerAvailable && puppeteerStealth ? puppeteerStealth.getBrowserStats() : null
    },
    vpn: {
      controlUrl: GLUETUN_CONTROL_URL,
      proxyEnabled: puppeteerAvailable && puppeteerStealth ? puppeteerStealth.USE_VPN_PROXY : false,
      proxyUrl: puppeteerAvailable && puppeteerStealth ? puppeteerStealth.VPN_PROXY_URL : null
    },
    circuitBreaker: {
      open: amazonCircuitOpen,
      consecutiveFailures,
      maxFailures: MAX_FAILURES_BEFORE_CIRCUIT_OPEN
    }
  };
}
...
function isRobotDetected(html) {
  ...
  // Indicateurs précis de captcha/blocage (sans "robot" qui cause des faux positifs)
  return html.includes("api-services-support@amazon.com") 
      ...
      || html.includes("Toutes nos excuses"); // Page d'erreur Amazon FR
}
...
// ============================================================================
// FONCTION PRINCIPALE - Puppeteer Stealth via proxy VPN
// ============================================================================

/**
 * Récupère une page Amazon via Puppeteer Stealth + proxy VPN
 * Tout le trafic passe par le VPN Gluetun pour masquer l'IP réelle
 * 
 * @param {string} url - URL Amazon à charger
 * @param {number} timeout - Timeout en ms
 * @returns {Promise<string>} - HTML de la page
 */
async function amazonGet(url, timeout = 45000) {
  if (!puppeteerAvailable || !puppeteerStealth) {
    throw new Error('Puppeteer Stealth non disponible - Amazon désactivé');
  }

  try {
    log.debug('🥷 Utilisation de Puppeteer Stealth via VPN...');
    const html = await puppeteerStealth.amazonStealthSearch(url, { timeout, retries: 2 });
    
    // Debug: vérifier le contenu
    const hasContent = html && html.length > 1000;
    const isBlocked = isRobotDetected(html);
    log.debug(`🔍 Puppeteer: ${hasContent ? html.length + ' chars' : 'vide'}, robot=${isBlocked}`);
    
    if (hasContent && !isBlocked) {
      log.debug('✅ Puppeteer Stealth: succès');
      return html;
    }
    
    // Échec - afficher un extrait pour debug
    if (isBlocked && html) {
      log.debug(`🔍 Extrait HTML: ${html.substring(0, 300).replace(/\s+/g, ' ')}`);
    }
    
    throw new Error(isBlocked ? 'Amazon a détecté un robot' : 'Réponse vide ou invalide');
    
  } catch (err) {
    log.warn(`⚠️ Puppeteer Stealth échoué: ${err.message}`);
    throw err;
  }
}
...
function parseSearchResults(html, country = "fr") {
  ...
  for (const asin of asins.slice(0, 30)) { // Max 30 résultats
    ...
    // Extraire la plateforme depuis le titre pour les jeux vidéo
    // Patterns courants: "- Nintendo Switch", "- PS5", "- PlayStation 5", etc.
    if (product.title) {
      const platformPatterns = [
        /[-–]\s*(Nintendo Switch\s*2?|Switch\s*2?)/i,
        /[-–]\s*(PlayStation\s*[45]|PS[45])/i,
        /[-–]\s*(Xbox\s*(?:Series\s*[XS]|One))/i,
        /[-–]\s*(PC)/i,
        /[-–]\s*(Wii\s*U?)/i,
        /[-–]\s*(3DS|2DS)/i,
        /\(?(Nintendo Switch\s*2?|PS[45]|PlayStation\s*[45]|Xbox[^)]*)\)?$/i
      ];
      
      for (const pattern of platformPatterns) {
        const platformMatch = product.title.match(pattern);
        if (platformMatch) {
          product.platform = platformMatch[1].trim();
          break;
        }
      }
    }
...
function parseProductDetails(html, asin, country = "fr") {
  ...
  // ========================================================================
  // INFORMATIONS SPÉCIFIQUES JEUX VIDÉO
  // ========================================================================
  
  // Plateforme (ex: "Nintendo Switch 2", "PlayStation 5")
  // Format: <span class="a-text-bold"> Plate-forme  : </span>  Nintendo Switch 2
  const platformMatch = html.match(/Plate-forme\s*:\s*<\/span>\s*([^<|]+)/i) ||
                        html.match(/Platform\s*:\s*<\/span>\s*([^<|]+)/i) ||
                        html.match(/platformInformation_feature_div[^>]*>[\s\S]*?<\/span>\s*([^<|]+)/i);
  if (platformMatch) {
    product.platform = decodeHtmlEntities(platformMatch[1].trim());
  }
  
  // Extraire les infos de la liste à puces (PEGI, Date sortie, Nb joueurs)
  // Format: <ul class="a-unordered-list..."><li>...<span class="a-list-item">TEXTE</span></li>...</ul>
  const bulletListMatch = html.match(/<ul[^>]*class="[^"]*a-unordered-list[^"]*a-vertical[^"]*a-spacing-mini[^"]*"[^>]*>([\s\S]*?)<\/ul>/i);
  if (bulletListMatch) {
    const listHtml = bulletListMatch[1];
    const listItems = [];
    const itemRegex = /<span[^>]*class="a-list-item"[^>]*>\s*([^<]+)/gi;
    let itemMatch;
    ...
    for (const item of listItems) {
      // PEGI (ex: "Pegi: 12", "PEGI 16")
      const pegiMatch = item.match(/Pegi\s*:?\s*(\d+)/i);
      if (pegiMatch) {
        product.pegi = parseInt(pegiMatch[1]);
        product.minAge = product.pegi;
      }
      
      // Date de sortie (ex: "Sortie: le 5 juin 2025", "Release Date: June 5, 2025")
      const releaseDateMatch = item.match(/Sortie\s*:?\s*(?:le\s+)?(\d{1,2}\s+\w+\s+\d{4})/i) ||
                               item.match(/Release\s*(?:Date)?\s*:?\s*(\w+\s+\d{1,2},?\s+\d{4})/i);
      if (releaseDateMatch) {
        product.releaseDateText = releaseDateMatch[1].trim();
        // Essayer de parser la date
        try {
          const parsedDate = parseAmazonDate(releaseDateMatch[1].trim());
          if (parsedDate) {
            product.releaseDate = parsedDate;
          }
        } catch (e) { /* ignore */ }
      }
      
      // Nombre de joueurs (ex: "1 joueur", "1-4 joueurs", "2-8 players")
      const playersMatch = item.match(/(\d+(?:\s*-\s*\d+)?)\s*joueur/i) ||
                          item.match(/(\d+(?:\s*-\s*\d+)?)\s*player/i);
      if (playersMatch) {
        product.nbPlayersText = playersMatch[0].trim();
        const nbMatch = playersMatch[1].match(/(\d+)(?:\s*-\s*(\d+))?/);
        if (nbMatch) {
          product.minPlayers = parseInt(nbMatch[1]);
          product.maxPlayers = nbMatch[2] ? parseInt(nbMatch[2]) : parseInt(nbMatch[1]);
          product.isMultiplayer = product.maxPlayers > 1;
        }
      }
    }
    
    // Stocker les bullet points comme features
    if (listItems.length > 0) {
      product.features = listItems;
    }
  }
  
  // Prix amélioré - essayer plusieurs patterns
  if (!product.price || !product.priceValue) {
    // Pattern pour le prix dans le format a-price-whole + a-price-fraction
    const priceWholeMatch = html.match(/<span[^>]*class="[^"]*a-price-whole[^"]*"[^>]*>(\d+)/i);
    const priceFractionMatch = html.match(/<span[^>]*class="[^"]*a-price-fraction[^"]*"[^>]*>(\d+)/i);
    const priceSymbolMatch = html.match(/<span[^>]*class="[^"]*a-price-symbol[^"]*"[^>]*>([^<]+)/i);
    
    if (priceWholeMatch) {
      const whole = priceWholeMatch[1];
      const fraction = priceFractionMatch ? priceFractionMatch[1] : "00";
      const symbol = priceSymbolMatch ? priceSymbolMatch[1].trim() : "€";
      product.price = `${whole},${fraction}${symbol}`;
      product.priceValue = parseFloat(`${whole}.${fraction}`);
    }
    
    // Fallback: prix dans corePriceDisplay
    if (!product.priceValue) {
      const corePriceMatch = html.match(/id="corePriceDisplay_desktop_feature_div"[\s\S]*?class="[^"]*a-offscreen[^"]*"[^>]*>([^<]+)</i);
      if (corePriceMatch) {
        product.price = decodeHtmlEntities(corePriceMatch[1].trim());
        product.priceValue = parsePrice(product.price);
      }
    }
  }
...
/**
 * Parse une date Amazon en différentes langues
 * @param {string} dateStr - Date au format "5 juin 2025" ou "June 5, 2025"
...
function parseAmazonDate(dateStr) {
  ...
  if (frMatch) {
    ...
    const month = frenchMonths[monthStr] ?? englishMonths[monthStr];
  ...
  if (enMatch) {
    ...
    const month = englishMonths[monthStr] ?? frenchMonths[monthStr];
...
export async function searchAmazon(query, options = {}) {
  ...
  try {
    ...
    // Utilise Puppeteer Stealth en priorité, FlareSolverr en fallback
    const html = await executeWithRobotRetry(
      () => amazonGet(searchUrl, 60000),
...
export async function getAmazonProduct(asin, country = "fr") {
  ...
  // Extraire l'ASIN si le format est "amazon_*_ASIN" (ex: amazon_movies_B003OBZ6HE)
  let cleanAsin = asin;
  if (asin && asin.includes("_")) {
    const parts = asin.split("_");
    const lastPart = parts[parts.length - 1];
    if (/^[A-Z0-9]{10}$/.test(lastPart)) {
      cleanAsin = lastPart;
      log.debug(`ASIN extrait de ${asin}: ${cleanAsin}`);
    }
  }
  ...
  const productUrl = `${marketplace.baseUrl}/dp/${cleanAsin}`;
  const cacheKey = `amazon:product:${country}:${cleanAsin}`;
...
/**
 * Recherche de produits sur Amazon avec résultats normalisés
 * @param {string} query - Terme de recherche
 * @param {object} options - Options (country, category, page, limit)
 * @returns {Promise<object>} - Résultats normalisés
 */
export async function searchAmazonNormalized(query, options = {}) {
  const rawResult = await searchAmazon(query, options);
  return normalizeAmazonSearch(rawResult);
}

/**
 * Récupérer les détails d'un produit Amazon avec résultat normalisé
 * @param {string} asin - ASIN du produit
 * @param {string} country - Code pays
 * @returns {Promise<object>} - Détails normalisés
 */
export async function getAmazonProductNormalized(asin, country = "fr") {
  const rawResult = await getAmazonProduct(asin, country);
  return normalizeAmazonProductDetail(rawResult);
}

/**
 * Obtenir le meilleur prix parmi plusieurs pays avec résultat normalisé
 * @param {string} asin - ASIN du produit
 * @param {string[]} countries - Codes pays à comparer
 * @returns {Promise<object>} - Comparaison normalisée
 */
export async function comparePricesNormalized(asin, countries = ["fr", "us", "uk", "de", "es"]) {
  const rawResult = await comparePrices(asin, countries);
  return normalizeAmazonPriceComparison(rawResult);
}

/**
 * Recherche multi-pays simultanée avec résultats normalisés
 * @param {string} query - Terme de recherche
 * @param {string[]} countries - Codes pays
 * @param {object} options - Options supplémentaires
 * @returns {Promise<object>} - Résultats normalisés
 */
export async function searchMultiCountryNormalized(query, countries = ["fr", "us", "uk"], options = {}) {
  const rawResult = await searchMultiCountry(query, countries, options);
  return normalizeAmazonMultiCountrySearch(rawResult);
}
