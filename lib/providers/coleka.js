...
function deduplicateColekaImages(images) {
  ...
  for (const url of images) {
    ...
    // Ex: "coleka-attrap-souris-jeu-de-societe-pour-enfants-jeu-de-plateau-version-francaise"
    let baseName = url
      .replace(/^https?:\/\/(?:thumbs\.|www\.)?coleka\.com\//, '')  // Retirer le domaine
      .replace(/_\d+x\d+/, '')  // Retirer les dimensions (_470x246)
    ...
    if (!existing) {
      ...
      // Comparer les URLs - privilégier www.coleka.com sur thumbs.coleka.com
      const existingIsThumbnail = existing.includes('thumbs.coleka.com');
      const newIsThumbnail = url.includes('thumbs.coleka.com');
      
      // Garder l'original (non-thumbnail) si disponible
      if (existingIsThumbnail && !newIsThumbnail) {
        imageMap.set(baseName, url);
      }
      // Si les deux sont du même type, garder celui sans dimensions dans l'URL
...
export async function getColekaItemDetails(itemId, lang = "fr", retries = MAX_RETRIES) {
  ...
  while (attempt < retries) {
    ...
    try {
      ...
      // Vérifier si c'est une vraie page 404 (pas juste "404" quelque part dans le HTML)
      // On vérifie le titre de la page ou la présence d'un h1 valide
      ...
      // Dédupliquer les images en gardant la meilleure qualité
