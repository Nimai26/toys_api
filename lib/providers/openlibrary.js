...
/**
 * Parse une date OpenLibrary et extrait l'année
 * Formats supportés: "1990", "2018-03-28", "30/11/2022", "March 1990", "1990-01-01", etc.
...
function parseYearFromDate(dateStr) {
  ...
  // Format: YYYY ou YYYY-MM-DD ou YYYY-MM
 ...
 * @param {string[]} dates - Liste de dates au format OpenLibrary
 * @returns {string|null} - La date la plus ancienne (format original)
 */
...
/**
 * Recherche de livres par auteur sur OpenLibrary API
 * @param {string} author - Nom de l'auteur
 * @param {object} options - Options (lang, max)
 * @returns {Promise<object>} - Résultats de recherche
 */
export async function searchOpenLibraryByAuthor(author, options = {}) {
  const {
    lang = null,
    max = OPENLIBRARY_DEFAULT_MAX
  } = options;
  
  const limit = Math.min(Math.max(1, max), OPENLIBRARY_MAX_LIMIT);
  
  log.debug(`Recherche par auteur: "${author}" (lang: ${lang || 'any'}, max: ${limit})`);
  ...
  try {
    ...
    const totalDocs = data.numFound || 0;
    
    // Utilise le même parser que searchOpenLibraryByText
    const books = docs.map(doc => {
      ...
      return {
        ...
        src_url: olId ? `https://openlibrary.org/works/${olId}` : null,
        source: "openlibrary"
      };
    });
    
    log.debug(`✅ ${totalDocs} résultats trouvés, ${books.length} retournés`);
...
export async function getOpenLibraryById(olId) {
  ...
  try {
    ...
    if (data.type?.key === '/type/work') {
      ...
      if (data.authors && data.authors.length > 0) {
        const authorPromises = data.authors.slice(0, 5).map(async (authorRef) => {
          ...
          try {
            const authorResp = await fetchViaProxy(`${OPENLIBRARY_BASE_URL}${authorKey}.json`, {
      ...
      let language = null;
      let allPublishDates = [];  // Collecter toutes les dates pour trouver la plus ancienne
      
      try {
        const editionsUrl = `${OPENLIBRARY_BASE_URL}/works/${olId}/editions.json?limit=10`;
        const editionsResp = await fetchViaProxy(editionsUrl, {
          headers: { 'Accept': 'application/json', 'User-Agent': USER_AGENT }
        });
        ...
        if (editionsResp.ok) {
          ...
          for (const edition of editions) {
            ...
            // Collecter les dates de publication pour trouver la première édition
            if (edition.publish_date) {
              allPublishDates.push(edition.publish_date);
            }
          }
          
          // Si pas de first_publish_date, chercher la date la plus ancienne parmi les éditions
          if (!releaseDate && allPublishDates.length > 0) {
            releaseDate = findEarliestDate(allPublishDates);
          }
        }
      } catch (e) {
    ...
    if (data.works && data.works[0]?.key) {
      try {
        const workResp = await fetchViaProxy(`${OPENLIBRARY_BASE_URL}${data.works[0].key}.json`, {
          headers: { 'Accept': 'application/json', 'User-Agent': USER_AGENT }
        });
        if (workResp.ok) {
...
export async function getOpenLibraryByIdNormalized(olId, options = {}) {
  ...
  if (shouldTranslate && destLang) {
    ...
    if (normalized.genres && normalized.genres.length > 0) {
      try {
        const genresResult = await translateGenres(normalized.genres, destLang);
