...
export async function searchIgdb(query, clientId, accessToken, options = {}) {
  ...
  try {
    ...
    const response = await fetchViaProxy(`${IGDB_BASE_URL}/games`, {
      method: 'POST',
      headers: {
...
export async function getIgdbGameDetails(gameId, clientId, accessToken, options = {}) {
  ...
  try {
    ...
    const storylineOriginal = game.storyline || null;
    
    // Applique la traduction si demandée
    let finalSummary = summaryOriginal;
    let summaryTranslated = null;
    let finalStoryline = storylineOriginal;
    let storylineTranslated = null;
    ...
    if (shouldTranslate && destLang) {
      ...
      // Traduit le storyline
      if (storylineOriginal) {
        const result = await translateText(storylineOriginal, destLang, { enabled: true });
        if (result.translated) {
          finalStoryline = result.text;
          storylineTranslated = result.text;
        }
      }
    ...
    const result = {
      ...
      storyline: finalStoryline,
      storylineOriginal: storylineOriginal,
      storylineTranslated: storylineTranslated,
      rating: game.rating ? Math.round(game.rating * 10) / 10 : null,
      aggregatedRating: game.aggregated_rating ? Math.round(game.aggregated_rating * 10) / 10 : null,
      totalRating: game.total_rating ? Math.round(game.total_rating * 10) / 10 : null,
 ...
 */
export async function getIgdbGameDetailsNormalized(slug, clientId, accessToken, options = {}) {
  const rawResult = await getIgdbGameDetails(slug, clientId, accessToken, options);
