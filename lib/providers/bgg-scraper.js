...
import { FSR_BASE, API_BASE_URL } from '../config.js';
...
// Helper pour construire une URL proxy (absolue si API_BASE_URL défini, relative sinon)
function buildProxyUrl(targetUrl) {
  const proxyPath = `/bgg_scrape/proxy?url=${encodeURIComponent(targetUrl)}`;
  return API_BASE_URL ? `${API_BASE_URL}${proxyPath}` : proxyPath;
}
  ...
  try {
    ...
    while ((match = rowPattern.exec(html)) !== null) {
      ...
      if (imgSrcset) {
        ...
        imageProxy = buildProxyUrl(firstImg);
      }
    ...
    // Mettre à jour detailUrl avec le nom final (localisé ou traduit)
    // Permet à l'application cliente d'utiliser directement l'URL sans modifier son code
    results.forEach(game => {
      if (game.name) {
        game.detailUrl = `/bgg_scrape/details/${game.id}?name=${encodeURIComponent(game.name)}`;
      }
    });
...
export async function scrapeBGGDetails(bggId, options = {}) {
  ...
  try {
    ...
    const imageProxy = image ? buildProxyUrl(image) : null;
    ...
    if (autoTrad && lang !== 'en') {
      ...
      // NOTE: Les noms localisés ne sont pas disponibles directement dans le HTML de la page de détails BGG.
      // Les noms alternatifs (comme "Attrap'Souris" pour "Mouse Trap") apparaissent dans la RECHERCHE
      // quand on cherche avec le terme dans cette langue.
      // Pour le moment, on ne traduit PAS le nom littéralement (ce serait incorrect pour un nom de jeu).
      // Le nom reste en anglais sauf si l'utilisateur fait une recherche avec le nom local.
  ...
  try {
    ...
    while ((match = filePattern.exec(html)) !== null) {
      ...
      files.push({
        ...
        urlProxy: buildProxyUrl(`${BGG_BASE_URL}${path}`),
        downloadUrl: `${BGG_BASE_URL}/file/download/${fileId}`,
        downloadUrlProxy: buildProxyUrl(`${BGG_BASE_URL}/file/download/${fileId}`),
        type: fileType,
        language,
        isRules,
        thumbs
...
export {
  BGG_BASE_URL,
  BGG_DEFAULT_MAX,
  fsrGet  // Exporté pour utilisation dans le proxy
};
