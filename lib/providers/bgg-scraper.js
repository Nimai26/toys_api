...
function extractAlternateNamesFromHtml(html) {
  ...
  // Format: "alternatenames":["Nom1","Nom2",...]
  const altNamesJson = html.match(/"alternatenames"\s*:\s*\[([^\]]+)\]/i);
  if (altNamesJson) {
    try {
      const altArray = JSON.parse(`[${altNamesJson[1]}]`);
      ...
      const simpleMatches = altNamesJson[1].matchAll(/"([^"]+)"/g);
  ...
  // Méthode 2: Chercher dans le HTML les liens "Alternate Names"
  // Format: <a href="/boardgame/XXX/YYY/credits" ...>Alternate Names</a> suivi d'une liste
  const altSection = html.match(/Alternate\s+Names?[^<]*<\/[^>]+>[\s\S]*?<ul[^>]*>([\s\S]*?)<\/ul>/i);
  if (altSection) {
    const liMatches = altSection[1].matchAll(/<li[^>]*>([^<]+)<\/li>/gi);
    for (const m of liMatches) {
      const altName = decodeHtmlEntities(m[1]).trim();
      if (altName && altName.length > 1 && !names.some(n => n.name === altName)) {
        names.push({ name: altName, type: 'alternate' });
      }
    }
  }
  
  // Méthode 3: Extraire depuis les données de version/édition
  // Les noms localisés sont souvent liés aux versions
  const versionNames = html.matchAll(/"versionname"\s*:\s*"([^"]+)"/gi);
  for (const m of versionNames) {
    const vName = decodeHtmlEntities(m[1]).trim();
    if (vName && vName.length > 1 && !names.some(n => n.name === vName)) {
      names.push({ name: vName, type: 'alternate' });
    }
  }
