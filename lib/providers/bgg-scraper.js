...
const BGG_XML_API = 'https://boardgamegeek.com/xmlapi2/thing';
const BGG_DEFAULT_MAX = 20;
const BGG_RATE_LIMIT_MS = 2000; // 2 secondes entre requêtes (scraping = plus prudent)

// Mapping langue vers patterns de noms alternatifs BGG
// BGG ne fournit pas le code langue directement, on utilise des heuristiques
const LANG_NAME_HINTS = {
  fr: ['français', 'french', 'france', 'fr-fr', 'vf', 'belge', 'suisse', 'québec', 'attrap', 'souris'],
  de: ['deutsch', 'german', 'allemand', 'de-de', 'österreich', 'schweiz', 'fang die maus', 'mausefalle'],
  es: ['español', 'spanish', 'espagnol', 'es-es', 'ratonera', 'trampa para ratones'],
  it: ['italiano', 'italian', 'italien', 'it-it', 'trappola per topi'],
  pt: ['português', 'portuguese', 'brazil', 'brasil', 'pt-br', 'pt-pt'],
  nl: ['nederlands', 'dutch', 'nl-nl', 'muizenval'],
  pl: ['polski', 'polish', 'pl-pl', 'pułapka na myszy'],
  ja: ['japanese', '日本語', 'japan'],
  zh: ['chinese', '中文', 'china', 'taiwan'],
  ko: ['korean', '한국어', 'korea']
};
...
/**
 * Récupère les noms alternatifs d'un jeu via l'API XML BGG
 * L'API XML est accessible via FlareSolverr et retourne tous les noms alternatifs
 * @param {string|number} bggId - ID BGG du jeu
 * @returns {Promise<Array<{name: string, type: string}>>} - Liste des noms alternatifs
 */
async function fetchAlternateNames(bggId) {
  try {
    const xmlUrl = `${BGG_XML_API}?id=${bggId}&type=boardgame`;
    log.info(`📜 Récupération noms alternatifs via XML API: ${bggId}`);
    
    const xml = await fsrGet(xmlUrl, 30000);
    
    // Vérifier si on a bien du XML
    if (!xml || xml.length < 100) {
      log.warn(`⚠️ Réponse XML trop courte (${xml?.length || 0} chars) pour ${bggId}`);
      return [];
    }
    
    // Parser les noms depuis le XML
    // Format: <name type="primary" value="Mouse Trap"/>
    //         <name type="alternate" value="Attrap'Souris"/>
    const names = [];
    const namePattern = /<name\s+type="([^"]+)"\s+(?:sortindex="\d+"\s+)?value="([^"]+)"/gi;
    let match;
    
    while ((match = namePattern.exec(xml)) !== null) {
      names.push({
        name: decodeHtmlEntities(match[2]),
        type: match[1] // "primary" ou "alternate"
      });
    }
    
    log.info(`📋 ${names.length} noms trouvés pour ${bggId} (${names.filter(n => n.type === 'alternate').length} alternatifs)`);
    
    // Log des noms alternatifs pour debug
    if (names.length > 1) {
      const alternates = names.filter(n => n.type === 'alternate').slice(0, 5);
      log.info(`   Exemples: ${alternates.map(n => n.name).join(', ')}`);
    }
    
    return names;
    
  } catch (error) {
    log.warn(`⚠️ Impossible de récupérer les noms alternatifs: ${error.message}`);
    return [];
  }
}

/**
 * Trouve le meilleur nom localisé pour une langue cible
 * Utilise des heuristiques car BGG ne fournit pas le code langue des noms alternatifs
 * @param {Array<{name: string, type: string}>} names - Liste des noms
 * @param {string} targetLang - Langue cible (fr, de, es, etc.)
 * @returns {{localizedName: string|null, primaryName: string|null}}
 */
function findLocalizedName(names, targetLang) {
  if (!names || !names.length) {
    return { localizedName: null, primaryName: null };
  }
  
  // Trouver le nom principal (anglais généralement)
  const primaryEntry = names.find(n => n.type === 'primary');
  const primaryName = primaryEntry?.name || null;
  
  // Si la langue cible est l'anglais, retourner le nom principal
  if (targetLang === 'en') {
    return { localizedName: primaryName, primaryName };
  }
  
  // Obtenir les hints pour la langue cible
  const hints = LANG_NAME_HINTS[targetLang] || [];
  
  // Chercher un nom alternatif qui correspond à la langue cible
  // Stratégie 1: Chercher des caractères spécifiques à la langue
  const alternates = names.filter(n => n.type === 'alternate');
  
  // Patterns de caractères par langue
  const langCharPatterns = {
  ...
  // Stratégie 2: Chercher par patterns de caractères de la langue
  const charPattern = langCharPatterns[targetLang];
  if (charPattern) {
    for (const alt of alternates) {
      if (charPattern.test(alt.name)) {
        log.debug(`🌍 Nom localisé trouvé (pattern): "${alt.name}" pour ${targetLang}`);
        return { localizedName: alt.name, primaryName };
      }
    }
  }
  
  // Stratégie 3: Chercher par hints dans le nom (moins fiable)
  ...
  for (const alt of alternates) {
    ...
    for (const hint of lowerHints) {
      if (lowerName.includes(hint)) {
        log.debug(`🌍 Nom localisé trouvé (hint): "${alt.name}" pour ${targetLang}`);
        return { localizedName: alt.name, primaryName };
      }
    }
  }
  
  // Pas de nom localisé trouvé
  return { localizedName: null, primaryName };
}
...
export async function scrapeBGGDetails(bggId, options = {}) {
  ...
  try {
    ...
    // Localisation et traduction automatique si demandée
    if (autoTrad && lang !== 'en') {
      const destLang = extractLangCode(lang);
      
      // 1. Chercher le nom officiel localisé (via API XML BGG)
      // Les noms de jeux ne se traduisent PAS littéralement !
      // On cherche le nom officiel de la version locale (ex: "Attrap'Souris" pour la version française)
      try {
        const alternateNames = await fetchAlternateNames(bggId);
        const { localizedName, primaryName } = findLocalizedName(alternateNames, destLang);
        
        if (localizedName && localizedName !== primaryName) {
          result.nameOriginal = name; // Nom anglais original
          result.name = localizedName; // Nom officiel localisé
          result.nameLocalized = true; // Flag pour indiquer que c'est un nom officiel, pas une traduction
          log.info(`🌍 Nom localisé trouvé: "${name}" → "${localizedName}" (${destLang})`);
        }
        // Si pas de nom localisé trouvé, on garde le nom anglais (pas de traduction littérale)
      } catch (nameError) {
        log.warn(`⚠️ Recherche nom localisé échouée: ${nameError.message}`);
      }
      
      // 2. Traduire la description (celle-ci peut être traduite)
      if (description) {
        try {
      ...
      // 3. Traduire les catégories via dictionnaire (termsTranslated est le bon champ retourné)
      ...
      // 4. Traduire les mécaniques via dictionnaire
    ...
    log.info(`✅ Détails scrapés pour ${result.name} (${bggId})`);
