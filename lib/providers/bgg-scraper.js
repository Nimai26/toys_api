...
// Mapping langue vers patterns de caractères pour détecter les noms localisés
...
/**
 * Extrait les noms alternatifs depuis le HTML scrapé de la page BGG
 * Les noms sont dans les données JSON embarquées ou dans la section "Alternate Names"
 * @param {string} html - HTML de la page de détails BGG
 * @returns {Array<{name: string, type: string}>} - Liste des noms
 */
function extractAlternateNamesFromHtml(html) {
  const names = [];
  
  if (!html) return names;
  
  // Méthode 1: Extraire depuis les données JSON GEEK embarquées
  // Format possible: "alternatenames":["Nom1","Nom2",...] ou "alternate_names":[...]
  const altPatterns = [
    /"alternatenames"\s*:\s*\[([^\]]+)\]/i,
    /"alternate_names"\s*:\s*\[([^\]]+)\]/i,
    /"altnames"\s*:\s*\[([^\]]+)\]/i
  ];
  ...
  for (const pattern of altPatterns) {
    ...
    if (match) {
      log.info(`📋 Pattern trouvé: ${pattern.source.substring(0, 30)}...`);
      try {
        const altArray = JSON.parse(`[${match[1]}]`);
        for (const altName of altArray) {
          if (altName && typeof altName === 'string' && altName.length > 1) {
            names.push({ name: decodeHtmlEntities(altName), type: 'alternate' });
          }
        }
      } catch (e) {
        // Fallback: extraction par regex simple
        const simpleMatches = match[1].matchAll(/"([^"]+)"/g);
        for (const m of simpleMatches) {
          if (m[1] && m[1].length > 1) {
            names.push({ name: decodeHtmlEntities(m[1]), type: 'alternate' });
          }
        }
      }
      break;
  ...
  // Méthode 2: Chercher dans le HTML le bloc "Also known as" ou "Alternate"
  // Format BGG: souvent dans un bloc de texte ou table
  const alsoKnownPatterns = [
    /Also\s+[Kk]nown\s+[Aa]s[:\s]+([^<]+)/i,
    /Alternate\s+[Nn]ames?[:\s]+([^<]+)/i
  ];
  ...
  for (const pattern of alsoKnownPatterns) {
    ...
    if (match) {
      ...
      const altNames = altText.split(/[,;]/).map(n => n.trim()).filter(n => n.length > 1);
      for (const altName of altNames) {
        if (!names.some(n => n.name === altName)) {
          names.push({ name: decodeHtmlEntities(altName), type: 'alternate' });
        }
      }
    }
  ...
  // Méthode 3: Chercher les liens vers les versions qui contiennent souvent le nom localisé
  // Format: href="/boardgameversion/XXXXX/NOM-VERSION"
  const versionPattern = /href="\/boardgameversion\/\d+\/([^"]+)"/gi;
  let vMatch;
  while ((vMatch = versionPattern.exec(html)) !== null) {
    // Le slug contient le nom avec des tirets
    const versionSlug = decodeURIComponent(vMatch[1]);
    // Convertir les tirets en espaces et capitaliser
    const versionName = versionSlug.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
    // Ne garder que si ça contient des caractères non-ASCII (nom localisé)
    if (/[^\x00-\x7F]/.test(versionName) && !names.some(n => n.name === versionName)) {
      names.push({ name: versionName, type: 'alternate' });
    }
  }
  
  if (names.length > 0) {
    log.info(`📋 ${names.length} noms alternatifs extraits du HTML`);
  }
 ...
 * Utilise des heuristiques basées sur les caractères spéciaux de chaque langue
 * @param {Array<{name: string, type: string}>} names - Liste des noms
 * @param {string} targetLang - Langue cible (fr, de, es, etc.)
 * @param {string} primaryName - Nom principal (anglais) pour comparaison
 * @returns {string|null} - Nom localisé ou null si non trouvé
 */
function findLocalizedName(names, targetLang, primaryName) {
  if (!names || !names.length || targetLang === 'en') {
    return null;
  }
  
  // Utiliser le pattern de caractères de la langue cible
  const charPattern = LANG_CHAR_PATTERNS[targetLang];
  if (!charPattern) {
    return null;
  }
  
  // Chercher un nom alternatif avec des caractères de la langue cible
  for (const alt of names) {
    // Ignorer si c'est le même que le nom principal
    if (alt.name === primaryName) continue;
    
    // Vérifier si le nom contient des caractères de la langue cible
    if (charPattern.test(alt.name)) {
      log.info(`🌍 Nom localisé trouvé: "${alt.name}" pour ${targetLang}`);
      return alt.name;
    }
  }
...
export async function scrapeBGGDetails(bggId, options = {}) {
  ...
  try {
    ...
    if (autoTrad && lang !== 'en') {
      ...
      // 1. Chercher le nom officiel localisé depuis le HTML scrapé
      ...
      const alternateNames = extractAlternateNamesFromHtml(html);
      if (alternateNames.length > 0) {
        log.info(`📋 ${alternateNames.length} noms alternatifs trouvés pour ${bggId}`);
        const localizedName = findLocalizedName(alternateNames, destLang, name);
