...
export async function scrapeBGGSearch(query, options = {}) {
  const { max = BGG_DEFAULT_MAX, lang = 'fr', autoTrad = false } = options;
  ...
  try {
    ...
    while ((match = rowPattern.exec(html)) !== null) {
      ...
      // Extraire la meilleure URL d'image du srcset (prendre la première, format micro)
      // srcset contient: "url1 1x, url2 2x" - on prend la première
      let image = null;
      if (imgSrcset) {
        const firstImg = imgSrcset.split(',')[0].trim().split(' ')[0];
        // Convertir micro en thumb pour une meilleure qualité (64x64 -> 200x200)
        image = firstImg.replace('__micro/', '__thumb/').replace('/64x64/', '/200x200/');
      }
 ...
 * @param {object} options - Options (lang, autoTrad, includeManuals)
 * @returns {Promise<object>} - Détails du jeu
 */
export async function scrapeBGGDetails(bggId, options = {}) {
  const { lang = 'fr', autoTrad = false, includeManuals = true } = options;
  ...
  try {
    ...
    if (autoTrad && description && lang !== 'en') {
      try {
        const translationResult = await translateText(description, lang, { enabled: true, sourceLang: 'en' });
        ...
        log.warn(`⚠️ Traduction échouée: ${tradError.message}`);
      }
    }
 ...
 */
export async function scrapeBGGManuals(bggId, preferredLang = 'fr') {
  if (!bggId) {
    throw new Error('ID BGG requis');
  }
