...
import { normalizePlaymobilSearch, normalizePlaymobilDetail } from '../normalizers/construct-toy.js';
...
export async function getPlaymobilInstructions(productId) {
  ...
  try {
    ...
    const response = await fetchViaProxy(instructionsUrl, {
      method: 'HEAD',
      headers: {
...
/**
 * Recherche Playmobil avec retour normalisé v3.0.0
 * @param {string} searchTerm - Terme de recherche
 * @param {string} lang - Locale
 * @param {object} options - Options (maxResults, retries)
 * @returns {Promise<object>} - Résultats normalisés
 */
export async function searchPlaymobilNormalized(searchTerm, lang = DEFAULT_LOCALE, options = {}) {
  const { maxResults = 24, retries = MAX_RETRIES } = options;
  
  const raw = await searchPlaymobil(searchTerm, lang, retries, maxResults);
  
  return {
    results: (raw.products || []).map(p => normalizePlaymobilSearch(p)),
    total: raw.total || 0,
    count: raw.count || 0,
...
/**
 * Détails produit Playmobil avec retour normalisé v3.0.0
 * @param {string} productId - ID du produit
 * @param {string} lang - Locale
 * @param {number} retries - Nombre de tentatives
 * @returns {Promise<object>} - Détails normalisés
 */
export async function getPlaymobilProductDetailsNormalized(productId, lang = DEFAULT_LOCALE, retries = MAX_RETRIES) {
  const raw = await getPlaymobilProductDetails(productId, lang, retries);
  return normalizePlaymobilDetail(raw, lang);
}

// Export des fonctions de normalisation
export { normalizePlaymobilSearch, normalizePlaymobilDetail };
